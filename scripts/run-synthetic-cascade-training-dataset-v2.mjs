...
if (
  ...
  !outputRoot.startsWith(path.join(root, "data", "synthetic"))
) {
...
try {
  const module = await vite.ssrLoadModule(
    "/src/evals/syntheticCascadeTrainingDatasetV2.ts",
  );
  const dataset = module.createSyntheticCascadeTrainingDatasetV2();
  const validation = module.validateSyntheticCascadeTrainingDatasetV2(dataset);
  if (!validation.passed) {
    throw new Error(
      `Synthetic Cascade v2 validation failed: ${JSON.stringify(validation.validationCodes)}`,
    );
  }
  ...
  const seedSpecPath = path.join(
    ...
    "data",
    "seed-specs",
    "synthetic-cascade-explanations-v2.json",
  );
  ...
  if (
    ...
    throw new Error("Cascade v2 seed specification does not match generated dataset");
  }
  ...
  const manifest = {
    schemaVersion: "synthetic-cascade-training-manifest-v2",
    ...
    dataMode: "SYNTHETIC",
    targetTask: "verified-role-explanation-strict-json-contract-hardening",
    ...
    counts: {
      ...
      contractProfiles: validation.contractProfileCounts,
      promptInjectionCases: validation.promptInjectionCases,
      numericDisplayAnchors: validation.numericDisplayAnchors,
      citationAnchors: validation.citationAnchors,
    },
    privacy: {
    ...
    contaminationBoundary: {
      ...
      violationCount: validation.contaminationBoundaryViolationCount,
    },
    trainingBoundary: {
  ...
  const evidence = {
    ...
    priorEvaluationOutputStored: false,
  ...
  const evidencePath = path.join(
    artifactDirectory,
    "synthetic-cascade-training-dataset-v2-latest.json",
  );
  await writeFile(evidencePath, `${JSON.stringify(evidence, null, 2)}\n`, "utf8");

  console.log(
    `SYNTHETIC_CASCADE_TRAINING_DATASET_V2_PASS parents=${validation.parentCount} records=${validation.recordCount} ` +
      ...
      `displayAnchors=${validation.numericDisplayAnchors} citationAnchors=${validation.citationAnchors} ` +
      `privacyViolations=${validation.privacyViolationCount} contaminationViolations=${validation.contaminationBoundaryViolationCount} ` +
      `duplicates=${validation.exactDuplicateCount}`,
  );
