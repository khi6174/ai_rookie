...
2026-09-05 포트폴리오 마감 보정: 적용 계층에서 버전이 변경된 모든 계획의 미완료 배송지에 안내 초안을 하나씩 생성한다. 물량이관 후 수신 계획에 속한 배송지를 포함한다. 적용된 ETA 원문과 해당 계획 버전을 함께 보존하고, 안내 문장은 담당 기사의 시간대로 날짜와 시간을 표시한다. 실제 전송은 하지 않는다.

현 이관 엔진은 소속·작업량·계획 버전과 예상 종료시각을 갱신하지만 이관 배송지의 개별 ETA·순서·도로 구간을 새로 최적화하지 않는다. 안내 초안은 이 합성 계획의 저장 ETA를 반영하며 실제 운행 일정의 실현 가능성을 입증하지 않는다. 전체 이관 일정 재계산은 종료 범위 밖이다.
