...
- `Demo pre-consented` 수신 기사 대신 별도 수신 기사 화면을 만들지 여부

이 정책이 `Approved`가 되기 전까지 추천 가중치, 동의 만료와 이관 바닥값은 구현의 확정 기준이 아니다.
