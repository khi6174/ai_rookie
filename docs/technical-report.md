...
**Technical Report · AI ROOKIE 2026 · 김용우 (대회 팀명: 안전빵)**

문서 상태: Approved · 담당: 김용우 · 최종 갱신: 2026-09-05. 포트폴리오 마감 기준이며, 대회 제출 당시 원문은 [v1.0.0 README](https://github.com/khi6174/ai_rookie/blob/v1.0.0/README.md)에 보존한다. 결정·미결사항·수용기준·비목표는 [마감 기록](./portfolio-closeout.md)을 따른다.

[![Release](https://img.shields.io/badge/Submission_Archive-v1.0.0-101C2C?style=flat-square)](https://github.com/khi6174/ai_rookie/tree/v1.0.0)
...
[![Unit Tests](https://img.shields.io/badge/Vitest-467%2F467-2E5D73?style=flat-square)](../artifacts/evals/unit-summary.json)
[![E2E](https://img.shields.io/badge/Playwright-70%2F70-2E5D73?style=flat-square)](../artifacts/evals/final-readiness-latest.json)
[![Data](https://img.shields.io/badge/Data-Synthetic_Demo-6F7782?style=flat-square)](./privacy-and-ai-policy.md)

[대회 제출 시점 v1.0.0 소스](https://github.com/khi6174/ai_rookie/tree/v1.0.0) · [공개 데모](https://saferoute-ai-demo.khiyw.chatgpt.site/) · [3분 시연 시나리오](./final-recording-script.md) · [제품 명세](./product-spec.md) · [평가 증거](../artifacts/evals/final-readiness-latest.json)
...
본 시스템은 남은 배송계획을 따라 `Safety Budget`과 `Time-to-Breach`를 결정론적으로 계산하고, 휴식·물량이관·배송순서 변경·안전경로·Safe Delay의 반사실적 결과를 비교한다. 안전하지 않은 후보는 ETA와 비용을 비교하기 전에 제외하며, 물량을 받는 기사까지 다시 계산하는 `Risk Transfer Guard`로 위험전가를 차단한다. 선택된 합성 계획은 영향 기사들의 동의와 관리자 승인을 거친 뒤 활성 계획과 고객안내 초안에 원자적으로 반영된다. 단, 물량이관은 소속과 작업량 등을 갱신하며 이관 배송지의 개별 ETA·순서·도로 구간을 새로 최적화하지 않는다. 국내 AI 계층은 검증된 수치와 문서 근거를 역할별 언어로 설명할 수 있지만, 안전 수치·실행 가능성·추천·적용 상태를 계산하거나 덮어쓸 수 없다.

30개 동결 합성 변형의 90회 전략 비교에서 SafeRoute는 하드 제약 위반 0건을 기록했으며, Fastest-only와 Balanced-only는 각각 17건과 11건을 기록했다. 최신 저장 증거 기준으로 단위·계약 테스트 467/467, Playwright E2E 70/70, Risk Transfer 경계 23/23, 결정 상태기계 경계 30/30, clean start 3/3을 통과했다. 다만 관리자 공간 이해도 Round 3은 0/6으로 실패했고 후속 독립 검토가 완료되지 않았다. 따라서 현재 결과는 **기술 데모 제출 가능, 관리자 이해도 검증 공백 공개** 상태다.
...
`Time-to-Breach`는 최대 120분의 예측 구간에서 `B < 30`이 되는 첫 시점이다. 임계치 판정은 반올림 전 값으로 수행하며, 결측 입력이 늘어날수록 신뢰도가 높아지지 않는 단조성을 강제한다. 자세한 공식과 한계는 [Safety Budget 모델 명세](./safety-model.md)에 있다.
...
- **전략 비교:** 30개 frozen 변형 × Fastest-only·Balanced-only·SafeRoute = 90회. Fastest-only는 ETA, Balanced-only는 기사 간 배송 건수의 최대–최소 차이를 최소화한다. Balanced-only는 안전과 ETA의 가중합이 아니다.
...
평가 계획, 데이터 분할, 수용 기준은 [평가·검증 계획](./evals.md)에서 관리한다.

## 6. 결과

### 6.1 기술 검증

2026-09-05 마감 수정 후 실행 결과. 사람 검토·모델 학습·부하 프로파일 등 별도 연구의 저장 결과는 작성 당시 증거이며 이번에 다시 수집한 현장 결과가 아니다.

| 평가 | 결과 | 저장 증거 |
|---|---:|---|
| Vitest 단위·계약 테스트 | **467 / 467** | [unit-summary.json](../artifacts/evals/unit-summary.json) |
...
- Node.js 20.19+ 또는 22.12+ (Vite 요구 버전)
- 전체 E2E Gate에는 Windows PowerShell과 Playwright Chromium 설치가 필요하다.
- pnpm
...
핵심 구현 경계는 [아키텍처](./architecture.md), 지속 결정은 [ADR 기록](./decisions.md), 개인정보·AI 권한은 [정책 문서](./privacy-and-ai-policy.md)에서 확인할 수 있다.
...
4. 실제 기사 업무 데이터·TMS 쓰기·인증·고객 메시지 발송은 연결하지 않았다. 기사 화면에는 사용자가 직접 켜는 기기 위치 표시가 있으며, 위치는 기기 메모리에서만 사용하고 서버·관리자·Safety 계산에 보내지 않는다.
5. 기상청·TAAS Live 표본은 출처 검증과 지역 맥락에만 사용하며 Safety 입력으로 자동 승격하지 않는다.
6. 관리자 공간 이해도 재검증과 역할별 독립 사용성 검토가 남아 있다.
7. 이관 엔진은 개별 ETA·순서·도로 구간을 재최적화하지 않는다. 현재 ETA는 합성 계획 값이므로 실제 도로에서 실현 가능한 일정이라고 주장하지 않는다.
8. 프로젝트는 개인 포트폴리오 기술 데모로 종료한다. 현장 파일럿·추가 서비스 개발은 이 프로젝트의 후속 약속이 아니다.
...
- [제품 명세](./product-spec.md)
- [데이터 계약](./data-contracts.md)
- [Safety Budget 모델](./safety-model.md)
- [개입·승인 정책](./intervention-policy.md)
- [개인정보·책임 있는 AI 정책](./privacy-and-ai-policy.md)
- [아키텍처](./architecture.md)
- [평가·검증 계획](./evals.md)
- [최종 녹화 시나리오](./final-recording-script.md)
