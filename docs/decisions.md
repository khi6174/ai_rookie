...
- 결정: 기사 앱의 `응급 상황 전송`은 현재 합성 `courierId`와 고정 출처 명령만 D1 `operations_rider_danger_signals`에 저장한다. 서버가 고정 문구, 수신시각과 15분 만료를 생성하며 공개 관제는 1초 `ETag` 조건부 조회로 열린 화면을 갱신한다. 브라우저 저장소·이벤트는 공유 저장 실패 때의 비권위 Fallback으로만 남긴다. 관제 지도는 정규화 운영 데이터의 실제 허브 ID `demo-hub-01/02/03`을 각각 북부·남부·서부 anchor에 연결한다. 허브 배지는 비대화형 표시이므로 `pointer-events: none`을 적용하고 Kakao가 생성하는 외부 overlay wrapper도 기사 마커보다 낮은 z-index에 둔다. 포인터가 지도 안에 있거나 지도 마커에 키보드 포커스가 있는 동안에는 React 상태 변경 없이 ref로 현재 합성 이동 프레임을 고정하고, 마커의 `pointerdown`에서 지도 이벤트 전파 전에 선택을 확정한다. 포인터·포커스가 지도 밖으로 나가면 1초 갱신을 재개한다.
...
### ADR-142 — 현재 공개 Demo 대시보드를 시각 기준선으로 동결하고 가산형으로 개선한다

- 날짜: 2026-08-06
- 상태: Superseded by ADR-146
- 사용자 결정: 현재 Demo 대시보드가 좋다는 외부 평가를 확인했으며, 전면 리디자인 대신 현재 화면을 유지하면서 기능을 덧대는 방향을 승인했다.
- 결정: 공개 루트 `/`의 현재 `Safety Control Tower` 정보구조, 밝은 색상 체계, 기사 카드·지도·지원 큐의 선택 동기화와 같은 화면의 지원 검토 폐루프를 제출 Demo의 시각·상호작용 기준선으로 동결한다. `/operations`, `/rider-demo`, `/stage`는 각각 상세 운영, 기사, 발표 보조 경로로 보존한다. 이후 개선은 기존 카드·패널·모달 안의 작은 상태 표시, 접을 수 있는 근거, 출처·감사 정보, Loading·Fallback·Error·Offline 상태와 접근성 보강처럼 현재 흐름을 깨지 않는 가산형 변경만 허용한다.
- AI 표시 경계: A.X v2의 12/12 Local 자격 결과는 검증 증거나 비교 정보로만 추가할 수 있으며 현재 공개 runtime이 Local 모델을 사용한다고 표시하지 않는다. 실제 제품 설명 경계는 ADR-141에 따라 Hosted·Template를 유지한다. AI 문구는 결정론적 Safety 수치·후보 실행 가능성·추천·적용 상태를 바꾸지 않는다.
- 성능 기준: 공개 지원 검토 안의 검증 설명 출처·Live/Fallback 표시와 공유 입력 생성기의 비공간 증가분은 직전 승인 build 대비 gzip JS `1,470 bytes`다. 이를 G5 비교 baseline에 분리해 키 없는 기준을 `146,988 bytes`로 재고정하며, 2.5D 장면의 추가 허용량 `50KiB`와 프레임·전환 시간 기준은 변경하거나 완화하지 않는다.
- 회귀 기준: 관리자 1440×900·1280×720, 기사 390×844·360×800에서 현재 주요 행동과 정보 우선순위를 보존한다. 지도·카드·지원 큐의 동일 기사 선택, 동일 decision ID, 기사 동의·수정·거절, 수신 기사 동의, 관리자 승인, 경로·순서·ETA·고객안내 갱신과 감사기록 E2E가 계속 통과해야 한다. 기능 추가 뒤 기존 결정론 테스트·E2E·production build를 다시 통과하지 못하면 공개하지 않는다.
- 이유: 현재 대시보드는 이미 사용자·외부 평가에서 강점이 확인됐고 P0 폐루프를 한 화면에서 재현한다. 제출 직전 구조와 시각 언어를 다시 바꾸면 학습 비용과 회귀 위험은 커지지만 핵심 가치 증명은 강해지지 않는다. 검증된 기준선을 유지하면 새 기능의 가치와 영향을 독립적으로 확인할 수 있다.
...
- 성능 기준: 사진 sprite는 별도 정적 JPEG로 제공하고 기사 등록 UI의 비공간 증가는 직전 승인 build 대비 gzip JS `1,050 bytes`다. 이를 G5 비교 baseline에 분리해 키 없는 기준을 `149,093 bytes`로 재고정하며, 2.5D 장면의 추가 허용량 `50KiB`와 프레임·전환 시간 기준은 변경하거나 완화하지 않는다.
...
### ADR-146 — 관리자 첫 화면을 향후 60분 판단 브리핑 중심으로 재구성한다

- 날짜: 2026-08-07
- 상태: Approved
- 사용자 결정: 이전 지도 중심 기준선을 고정하지 않고, 지금까지의 독립 이해도 실패와 기사 제품 경계 학습을 토대로 기획을 보완한 대시보드 재구성을 자율 위임했다. 이에 ADR-142의 가산형 변경 제한을 대체한다. ADR-145의 합성 사진·검증 전 기사 등록 경계는 계속 유지한다.
- 결정: 공개 루트 `/`의 첫 정보는 기사 카드나 지도가 아니라 `향후 60분 안전지원 브리핑`이다. 브리핑은 지원 판단이 필요한 기사 수, 선택한 지원받는 기사, 지원 시점·배송 순서, 현재에서 예상 최저 Safety Budget, 먼저 비교할 휴식·배송 분담, 위험전가 검사 범위와 `안전 후보 비교 → 영향 기사 확인 → 관리자 승인 후 적용` 순서를 한 영역에 고정한다. 합성 운영자료, 실제 GPS 아님, 사고확률·기사평가 아님, 승인 전 계획 유지 경계를 같은 첫 영역에서 읽을 수 있어야 한다.
- 정보위계: 지원 우선순위 목록은 지도보다 먼저 키보드·읽기 순서에 놓고, 지도는 선택 기사·허브·배송 맥락을 확인하는 보조 근거로 사용한다. 25명 전체 기사 현황은 첫 판단을 가리지 않도록 화면 하단의 압축 카드 rail로 이동한다. 지도·지원 목록·전체 카드의 선택 동기화와 같은 화면의 지원 검토 폐루프는 유지한다. 지원 검토는 `원 기사/수신 기사` 대신 `지원받는 기사/배송을 나눠 맡는 기사`를 사용하고, 먼저 확인할 결론과 사람 승인 전 계획 불변을 후보 표 위에 둔다.
- 안전·개인정보 경계: Safety Budget·Time-to-Breach·후보 실행 가능성·추천·Risk Transfer Guard·동의·승인·적용 상태는 기존 결정론 계약이 계속 소유한다. 실제 개인정보·사진 업로드·실제 GPS·생체정보·고객 발송·AI 결정 권한을 추가하지 않는다. 등록 대기 기사는 계속 활성 25명·지도·Safety 계산·D1에서 제외된다.
- 성능 기준: 판단 브리핑, 빠른 지원 시점 정렬과 재배치의 비공간 gzip JS 증가분은 직전 승인 build 대비 `863 bytes`다. 이를 G5 비교 baseline에 분리해 키 없는 기준을 `149,956 bytes`로 재고정하며, 2.5D 장면의 추가 허용량 `50KiB`와 프레임·전환 시간 기준은 변경하거나 완화하지 않는다. 첫 측정의 `50.76KiB` 초과는 숨기지 않고 비공간 증가분을 격리한 뒤 같은 장면 Gate를 다시 실행한다.
- 검토 경계: 화면과 지원 검토 자극이 바뀌므로 이전 `operations-service-human-review-v2` 관리자 화면 SHA-256은 더 이상 현재 공개 화면 근거가 아니다. 기존 사람 응답은 0건이므로 폐기할 결과는 없으며, 배포 후 새 관리자 자극·release commit·manifest를 생성해 관리자 3명·기사 5명 독립 검토를 처음부터 수행한다.
- 이유: 이전 구성은 25명 카드와 큰 지도가 첫 시선을 점유해 `누구에게 언제 어떤 지원이 필요한가`, 합성 데이터와 승인 경계, 지원받는 기사와 배송을 나눠 맡는 기사 영향을 사용자가 여러 영역에서 조합해야 했다. 과거 공간 이해도 Round 1~3에서는 핵심 시점·배송지·두 기사 영향·조치 순서를 한 번에 복원하지 못했다. 첫 결론을 문장과 구조화 사실로 고정하고 지도와 전체 현황을 보조 계층으로 내리면 제품의 실제 의사결정 순서와 화면 읽기 순서가 일치한다.
...
### ADR-148 — Shadow Live는 가명 진행 이벤트의 로컬 계약 검사부터 시작한다
...
- 배포 Gate: 공개 사람 검토 manifest의 `releaseCommit`이 실제 Sites source와 어긋나지 않도록 source commit을 만든 뒤 production build·패키징·버전 저장 순서를 지킨다. 이미 저장된 같은 commit의 잘못된 archive는 재사용하지 않고 새 결정 기록 commit에서 다시 빌드한다.
...
### ADR-151 — 외부 원천 대신 명시적인 합성 실시간 배송 진행을 재생한다

- 날짜: 2026-08-08
- 상태: Approved
- 사용자 결정: 택배사 TMS/WMS 원천 확보가 어려울 경우 가상 배송 데이터를 만들어 다음 단계로 진행하도록 요청했다. 합성 출처 표시는 마지막이나 별도 전달로 대체하고 싶다는 의견도 함께 제시했다.
- 결정: 외부 원천 없이 재현 가능한 `SYNTHETIC_STREAM` 배송 진행 generator와 수동 재생 UI를 구현한다. generator는 seed·시작시각·tick으로 가명 기사별 진행 이벤트를 만들며 기본 자동재생은 끈다. UI는 시작·일시정지·한 단계·초기화, 현재 진행률과 최근 이벤트를 제공한다.
- 출처 경계: 합성 표시는 마지막에만 두거나 발표자의 별도 설명으로 대체하지 않는다. 재생 카드와 공개 관리자 진입점에 `합성 실시간 재생 · 실제 TMS 아님` 또는 동등하게 명확한 문구를 가까이 유지한다. 반복되는 장문 한계 설명은 상세 영역으로 모을 수 있다.
- 데이터·권한 경계: 실제 이름·연락처·주소·고객·차량번호·GPS·좌표·생체정보를 생성하지 않는다. 재생 상태는 브라우저 메모리에만 두고 API·D1·AI·Safety·지원 큐·동의·승인·계획 적용에 연결하지 않는다. 기존 인증 필수 Shadow Live 서버 endpoint는 계속 비활성이고 합성 batch를 실제 원천으로 받지 않는다.
- 성능 기준: 합성 generator·재생 제어·가명 기사 진행·최근 이벤트 UI와 공개 관리자 진입 링크의 비공간 gzip JS 증가분은 직전 승인 build 대비 `1,730 bytes`다. 이를 G5 비교 baseline에 분리해 키 없는 기준선을 `155,422 bytes`로 재고정한다. 2.5D 장면의 추가 허용량 `50KiB`와 프레임·전환 시간 기준은 변경하거나 완화하지 않는다.
- 이유: 외부 협력 없이도 이벤트 순서·진행 표시·중복/역순 계약과 실시간 UI를 재현할 수 있지만, 합성 출처를 숨기면 실제 배송관제·현장검증·운영효과로 오인될 수 있다. 작고 지속적인 근접 표시는 시연 집중도와 정직한 데이터 경계를 함께 유지한다.
...
### ADR-155 — 현재 한계 초과와 미래 초과 예상을 분리하고 관리자 보류를 공개 폐루프에 연결한다

- 날짜: 2026-08-08
- 상태: Approved
- 근거: 최종 기능 대조에서 현재 Budget은 30 이상이지만 예상 최저만 30 미만인 기사에게 빨강 `한계 초과`가 표시되고, 반올림 전 값은 30 미만이지만 표시값은 `30.0`인 경계가 발견됐다. 공개 지원 검토에는 승인 행동만 있고 승인 문서와 상태기계가 요구하는 관리자 보류·재개가 연결되지 않았다.
- 결정: 현재 Budget이 30 미만인 경우만 빨강 `현재 한계 초과`로 표시한다. 현재는 30 이상이고 결정론적 forecast가 30 미만이면 앰버 `초과 예상`으로 표시하며, 소수 첫째 자리 반올림이 30.0이 되는 미만값은 `<30.0`으로 표시한다. 임계치와 반올림 전 밴드 판정은 변경하지 않는다. 지원 검토의 현재·예상 최저·Time-to-Breach·배송지·신뢰도·AI 설명 입력은 모달을 연 decision 스냅샷에서만 읽고 배경 합성 운행 값과 섞지 않는다.
- 관리자 분기: 두 기사 동의 뒤 관리자는 승인 전에 `보류`할 수 있고, `ADMIN_HELD` 동안 기존 계획을 유지하며 승인 버튼을 제공하지 않는다. `관리자 검토 다시 열기`는 같은 decision을 `ADMIN_APPROVAL_REQUIRED`로 되돌리고 이후 승인 시 기존 재검증·원자 적용 Gate를 그대로 수행한다. 저장 실패나 동시성 충돌은 보류·재개 성공으로 표시하지 않는다.
- 이유: 미래 지원 필요를 현재 사고·위험 상태처럼 표현하면 빨강 사용 원칙과 Time-to-Breach 의미가 무너진다. 또한 문서에만 있는 보류권은 공개 관리자 폐루프에서 실제 권리가 아니다. 계산을 바꾸지 않고 표시 의미와 기존 상태 전이를 연결하면 안전·동의 경계를 강화할 수 있다.
...
### ADR-160 — 공개 합성 실시간 화면의 최종 수동 점검 결함을 닫는다

- 날짜: 2026-08-09
- 상태: Approved
- 근거: 공개 배포 화면의 전체 조작 점검에서 첫 프로필 스프라이트가 20명만 포함해 21~25번 기사 카드가 숫자 대체 이미지로 남았고, 직접 기사 앱에는 ADR-154에서 제거한 `초기화`가 남아 있었다. 또한 기사 위험신호는 D1 저장 결과를 화면에 알리지 않았고 0건인 `위험신호` 필터는 클릭 가능한 무동작 상태였다.
- 결정: 합성 기사 21~25번용 명시적 합성 프로필 스프라이트를 추가해 25명 전원을 사진으로 표시한다. 직접 기사 앱에서는 `초기화`를 제거하되 개발용 기존 폐루프 Demo의 재현 reset은 유지한다. 위험신호는 실제 D1 저장 응답을 기다린 뒤 전송 중·성공·실패 상태를 기사에게 표시한다. 위험신호가 0건이면 해당 필터를 비활성화한다.
- 데이터·권리 경계: 새 이미지는 실제 기사가 아닌 AI 생성 합성 인물이며 신원·평가 입력으로 사용하지 않는다. 위험신호 성공 문구는 D1 저장 완료 전에는 표시하지 않고, 저장 실패를 성공처럼 대체하지 않는다. 실제 기기 위치 권한·정밀 GPS·새 외부 데이터는 이 변경에서 요청하거나 저장하지 않는다.
- 배포 Gate: 공개 검토 manifest는 빌드 시점의 Git HEAD를 기록하므로 최종 소스 commit 뒤 production build·패키징·버전 저장 순서를 지킨다. 이전 HEAD에서 만든 archive를 새 commit의 배포물로 재사용하지 않는다.
- 이유: 숫자 대체 이미지는 공개 화면의 미완성 인상을 만들고, 연속 서비스에 남은 reset과 결과 없는 전송은 사용자가 실제 상태를 오해하게 한다. 저장 결과와 조작 가능 상태를 화면에 정확히 연결하면 서비스 연속성과 폐루프 신뢰도를 함께 높인다.
...
### ADR-163 — Scenario 공개 이동에서 고정 폐루프를 제거하고 무그림자 디자인을 고정한다
...
### ADR-167 — 운행 갱신과 지도 카메라를 분리하고 기사 마커 충돌을 해소한다

- 날짜: 2026-08-10
- 상태: Approved
- 사용자 결정: 지도 확대·축소가 반복해서 초기화되는 현상을 제거하고, 기사 마커는 최대한 겹치지 않게 하되 불가피한 경우에도 한 줄에 2~3명만 배치하도록 요청했다.
- 결정: Kakao 지도 `setBounds`는 최초 지도 준비, 명시적 `배송구역 확대`·`전체 보기`, 기사 확대 상태에서의 선택 변경에만 호출한다. 배송 완료 수·운행 활동·Safety 갱신은 상세 overlay와 마커만 갱신하고 현재 카메라를 보존한다. 겹침 후보는 경로 좌표와 원시 위경도를 변경하지 않은 채 화면 표시용 결정론적 오프셋을 최대 3열로 적용하며 Kakao와 지도 대체 화면이 같은 배치 함수를 사용한다.
- 데이터·안전 경계: 표시 오프셋은 지도 UI에만 존재하고 Safety Budget, Time-to-Breach, 배송 순서, 감사기록, 위치 데이터셋과 서버 저장에 전달하지 않는다. 마커의 `data-latitude`·`data-longitude`는 원래 경로 위치를 계속 나타낸다.
- 검증: 단위 테스트는 선택 변화가 fleet 카메라 key를 바꾸지 않는지, 밀집 9명도 중복 슬롯 없이 최대 3열로 분리되는지 확인한다. E2E는 지도 대체 화면의 25명 마커와 한 지점의 동시 겹침 수가 3 이하인지 검사한다.
- 이유: 실시간 운영 수치 갱신이 사용자의 공간 탐색을 중단하면 지도 조작이 불안정하게 느껴지고, 다수 마커가 같은 위치에서 입력을 가리면 특정 기사를 직접 선택할 수 없다.
...
- 날짜: 2026-08-12
- 결정: 기사 카드의 Budget은 소수 첫째 자리 숫자로만 표시하고, 30 미만 값이 반올림으로 `30.0`이 되지 않도록 해당 경계 아래에서 보수적으로 절삭한다. 날짜·시간 선택은 업무·안전여유 기준 상황을 바꾸지 않는다. ASOS `관측 반영하고 예측`은 선택한 기준 상황을 유지한 채 강수·시정 입력과 결정론적 예상 최저·안전한계 시점·추천의 전후를 같은 화면에서 즉시 다시 계산해 표시한다.
...
### ADR-171 — 관측 선택과 시뮬레이션 가정을 1-1·1-2로 분리한다

- 날짜: 2026-08-12
- 결정: 예측 입력을 `1-1 관측 시점 선택`과 `1-2 시뮬레이션 조건 설정`으로 별도 카드에 구분한다. 날짜·시간과 ASOS 관측은 1-1, 대표 상황·업무·안전여유·기상·경로 가정은 1-2가 소유한다. 1-1에서 적용한 강수·시정은 1-2에 출처 표식과 전후 결과를 표시하지만 최종 예상 결과는 하나로 유지한다.
- 이유: 날짜 선택과 업무 가정이 서로를 자동으로 바꾸는 것처럼 읽히는 문제를 없애고, 관측값과 사용자가 설정한 가정을 같은 예측 안에서 추적 가능하게 하기 위해서다.
- 기각한 대안: 결과까지 두 개의 독립 예측으로 나누면 동일 계획에 상충하는 Safety 수치가 생길 수 있어 기각했다. 날짜 선택만 시각적으로 분리하고 관측 출처를 표시하지 않는 방식도 입력 유래를 알 수 없어 기각했다.
- 영향 파일: `src/ui/ScenarioPlanningLab.tsx`, `src/ui/scenario-planning.css`, Scenario E2E·화면 증거·승인 문서

### ADR-172 — 관측값은 날짜·시간 선택과 함께 1-2에 즉시 설정한다

- 날짜: 2026-08-12
- 결정: 1-1의 별도 `강수·시정을 1-2에 적용` 버튼을 제거한다. 날짜·시간에 대응하는 ASOS 관측이 있으면 강수·시정을 즉시 1-2 입력과 예상 결과에 반영한다. 선택 시간과 관측값은 두 열로 배치해 관측 카드가 남은 가로 폭을 사용한다.
- 이유: 관측 시점을 이미 선택한 뒤 같은 값을 다시 적용하는 중복 조작을 없애고 1-2 조건 설정으로 자연스럽게 이어지게 하기 위해서다.
- 기각한 대안: 버튼만 제거하고 관측값을 계산에 반영하지 않는 방식은 화면 문구와 실제 예측 입력이 달라져 기각했다.
- 영향 파일: `src/ui/ScenarioPlanningLab.tsx`, `src/ui/scenario-planning.css`, Scenario E2E·화면 증거·승인 문서

### ADR-173 — 관측 기반 예측과 가상 기상 스트레스 테스트를 분리한다

- 날짜: 2026-08-12
- 결정: Scenario 기본 모드는 선택 시점의 ASOS 관측 강수·시정을 사용하는 `ASOS 관측 기반`으로 둔다. 관측·시간·경로 조건을 한 번에 바꾸는 `우천·경사`, `폭염·계단`, `야간·낯선 권역` 복합 프리셋은 화면에서 제거한다. 업무·안전여유와 배송 경로 조건은 항상 조정할 수 있고, 강수·체감온도·시정의 임의 입력은 사용자가 `가상 기상 스트레스 테스트`를 켠 경우에만 노출한다.
- 이유: 관측값과 가정값이 충돌하는 숨은 덮어쓰기를 없애고, 실제 관측 기반 재현과 의도적인 반사실적 스트레스 테스트의 출처를 분명히 하기 위해서다.
- 기각한 대안: 복합 프리셋을 그대로 두고 설명 문구만 추가하는 방식은 하나의 버튼이 서로 다른 출처의 입력을 동시에 변경하는 문제를 해결하지 못해 기각했다.
- 영향 파일: `src/ui/ScenarioPlanningLab.tsx`, `src/ui/scenario-planning.css`, Scenario E2E·화면 증거·승인 문서

### ADR-174 — Scenario 선택 입력을 날짜·시간과 업무·안전여유로 제한한다

- 날짜: 2026-08-12
- 결정: 사용자 선택 입력은 `1-1 날짜·시간`과 `1-2 남은 배송·총 근무·연속작업·현재 안전여유·분담 기사 여유`만 남긴다. 권역 숙련도, 배송 경로 조건, 가상 기상 스트레스 테스트는 화면에서 제거한다. 선택 날짜·시간의 ASOS 강수·시정은 자동 반영하고 경로 조건·체감온도는 시연 기준계획의 고정값을 사용한다고 표시한다.
- 이유: 관제 사용자가 핵심 운영 예측을 빠르게 실행하도록 선택 부담을 줄이고, 관측값과 임의 가정을 다시 혼합하지 않기 위해서다.
- 기각한 대안: 고급 설정 접기 영역을 유지하는 방식은 실제 사용 빈도가 낮은 입력을 계속 의사결정 요소로 인식하게 만들어 기각했다.
- 영향 파일: `src/ui/ScenarioPlanningLab.tsx`, `src/ui/scenario-planning.css`, Scenario E2E·화면 증거·승인 문서

### ADR-175 — 현재 자원을 개입 비교 아래 오른쪽 열에 배치한다

- 날짜: 2026-08-12
- 결정: `04 현재 자원`을 페이지 전체 너비 하단에서 제거하고 오른쪽 결과 카드의 `03 개입 비교` 바로 아래에 배치한다. 데스크톱의 왼쪽 입력 카드와 오른쪽 결과 카드는 같은 상단선·높이, 20px 열 간격을 사용하며 현재 자원은 오른쪽 폭에 맞춘 2×2 그리드다.
- 이유: 입력 단순화 이후 생긴 오른쪽 하단 여백을 활용하고 두 주요 카드의 외곽 비율과 마진을 맞추기 위해서다.
- 기각한 대안: 빈 여백을 유지한 채 두 카드의 최소 높이만 강제로 맞추는 방식은 정보 흐름과 화면 밀도를 개선하지 못해 기각했다.
- 영향 파일: `src/ui/ScenarioPlanningLab.tsx`, `src/ui/scenario-planning.css`, Scenario E2E·화면 증거·디자인 문서

### ADR-176 — 관측 입력의 무변화 결과를 이유와 함께 축약한다

- 날짜: 2026-08-12
- 결정: 관측 강수·시정을 반영해도 예상 최저, 안전한계 시점과 추천이 모두 동일하면 반복 전후 지표를 숨기고 `예측 결과 변화 없음`과 이유를 표시한다. 입력 변화 자체는 `1-2에 반영된 관측 입력`으로 계속 추적한다.
- 이유: 시정 `2.5km → 20.0km`처럼 입력은 바뀌었지만 모델의 저시정 위험 시작점 1km를 모두 벗어나 결과가 동일한 사례를 기능 오류로 오인하지 않게 하기 위해서다.
- 기각한 대안: 입력 변화가 있으면 결과 변화가 없어도 동일한 세 지표를 항상 표시하는 방식은 의미 없는 화살표를 결과처럼 강조해 기각했다.
- 영향 파일: `src/ui/ScenarioPlanningLab.tsx`, `src/ui/scenario-planning.css`, Scenario E2E·화면 증거·설계 문서

### ADR-177 — 관제와 기사 앱이 같은 Kakao 도로 경로와 합성 시각을 사용한다

- 날짜: 2026-08-12
- 결정: 기사 앱은 관제와 같은 기사 프로필, `fleet-demo` Kakao Mobility Directions 경로와 공유 합성 시각을 사용한다. 기사 Kakao 지도에 동일 도로 polyline을 표시하고 마커를 같은 progress 계산으로 움직인다. 공급자 실패 시 두 화면 모두 같은 결정론 경로로 전환한다. 관제와 기사 지도의 초기 축척은 기존보다 Kakao level 1단계 넓힌다.
- 이유: 동일 기사인데 관제는 실제 도로 geometry, 기사 앱은 단순 corridor를 사용하는 불일치를 없애고 강·건물·직선 횡단처럼 보이는 이동을 줄이기 위해서다.
- 경계: 위치는 합성 표현이며 실제 GPS·TMS 위치를 데이터베이스에 저장하거나 화면 사이에 전송하지 않는다. D1은 동일 기사 프로필만 제공하고 좌표·tick·frame은 저장하지 않는다. 지도 경로와 확대는 Safety 계산에 참여하지 않는다.
- 기각한 대안: 실제 위치를 D1에 저장해 동기화하는 방식은 현재 승인된 개인정보·정밀 위치 범위를 확대하므로 기각했다. 화면별 별도 경로 계산도 위치 불일치를 만들기 때문에 기각했다.
- 영향 파일: `src/ui/RiderLiveLocationMap.tsx`, `src/ui/OnePageDashboardDemo.tsx`, `src/adapters/maps/kakao.ts`, 지도·기사 E2E, 데이터·아키텍처·디자인 문서

### ADR-178 — 관제와 기사 앱의 도로 이동 속도를 함께 낮춘다

- 날짜: 2026-08-13
- 상태: Approved
- 사용자 결정: 전체 기사 이동이 빠르게 보이므로 관제와 기사 앱의 이동 속도를 함께 낮추도록 요청했다.
- 결정: 이동 중인 기사 마커의 도로 polyline 한쪽 끝 통과 시간을 공통 30초로 둔다. 1초 운행 frame, 5초 Safety 재평가와 배송·휴식·지연 활동 주기는 유지하고, 위치 progress만 같은 계수로 줄인다. Kakao 도로 geometry와 결정론적 Fallback 모두 이 속도를 사용한다.
- 이유: 수치와 상태 갱신의 반응성은 유지하면서 차량 마커가 도로 위를 급하게 미끄러지는 인상을 줄이고, 관제와 기사 앱의 같은 기사 위치를 계속 일치시키기 위해서다.
- 기각한 대안: 전체 tick을 2초 이상으로 늦추면 배송·Safety 상태 갱신까지 느려지므로 기각했다. 관제만 CSS transition으로 늦추는 방식은 기사 앱과 같은 시각·위치를 깨뜨려 기각했다.
- 영향 파일: `src/application/riderMapPresentation.ts`, `src/application/syntheticLiveOperations.ts`, 관련 단위 테스트, 제품·아키텍처·디자인 문서

### ADR-179 — 배송 분담 전 과정에 수신 기사 이름을 표시한다

- 날짜: 2026-08-13
- 상태: Approved
- 사용자 결정: 배송을 넘길 때 누가 분담받는지 관제 화면에서 확인할 수 있도록 요청했다.
- 결정: 배송 분담 후보, 선택 요약, 위험전가 검사, 수신 기사 응답 대기와 적용 결과에 동일한 수신 기사 표시명을 노출한다. 이름은 선택된 `recipientCourierId`를 현재 검증된 운영 패키지의 가명 기사 디렉터리로 해석하며 별도 값을 만들지 않는다.
- 이유: 수신 기사 이름이 숨겨지면 관리자가 누구의 안전여유를 검사하고 누구의 동의를 기다리는지 확인할 수 없다. 동일 ID에서 해석한 이름을 전 단계에 유지해 결정 근거와 기사 앱 전환 대상을 명확히 한다.
...
### ADR-183 — 고객안내 결함을 보정하고 개인 포트폴리오로 마감한다

- 날짜: 2026-09-05
- 상태: Approved
- 사용자 결정: 결선 미진출 후 실서비스·창업 확장 없이 포트폴리오로 마감한다. 최종 점검의 보정 작업을 진행하도록 요청했으며 김용우가 기획·설계·개발·모델 학습·검증·발표 전 과정을 담당했다고 확인했다.
- 결정: 안내 대상을 적용 계층에서 변경된 계획의 모든 미완료 배송지로 확정하고 배송지별 ID를 직접 생성한다. 수신 계획의 이관 배송지도 포함한다. 안내 버전은 실제 소속 계획에 연결하고 UTC ETA는 보존하면서 사용자 시간대로 날짜·시간을 표시한다. 안내는 PREVIEW_ONLY를 유지한다.
- 이유: 출발 계획만 조회하던 구현이 이관 배송지를 누락했고 UTC 문자열을 현지 시각처럼 표시했다. 대상 생성과 초안 생성을 한 계층으로 모아 누락·순서 의존을 제거한다.
- 포트폴리오 결정: 대회 제출 원본과 v1.0.0 태그는 보존한다. 별도 마감 문서에서 비교 실험의 Balanced-only를 배송 건수 불균형 최소화로 바로잡고, 이관 일정 재계산·현장 이해도 검증의 한계를 공개한다. 원본의 작성 당시 수치를 최신 결과로 덮어쓰지 않는다.
- 기각한 대안: 호출부별 대상 목록 패치는 다른 호출 경로의 누락을 남긴다. 실제 알림 전송·경로 최적화 확대·현장 파일럿은 사용자가 정한 종료 범위를 넘으므로 수행하지 않는다.
- 수용기준: 이관 배송지 포함, 무관·완료 배송지 제외, 배송지별 단일 안내, 적용 ETA·계획 버전 일치, 한국 날짜 경계와 비한국 시간대 표시, 원자성·멱등성 회귀 및 기존 최종 Gate 통과.
- 영향 파일: `src/application/apply-plan/index.ts`, 해당 호출부·테스트, `docs/data-contracts.md`, `docs/intervention-policy.md`, `README.md`, 포트폴리오 마감 문서.
- 미결사항: 없음. 실서비스 검증과 확장은 비목표로 종료한다.
