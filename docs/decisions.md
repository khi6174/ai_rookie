...
- 날짜: 2026-07-28
- 상태: Approved
- 결정: 관리자 `/operations`의 64px 전역 헤더는 `#00BFFF`를 브랜드 배경으로 사용하고 글자·아이콘은 `#101828` 계열로 표시한다. `#00BFFF`는 브랜드·탐색·선택에만 사용하며 안정·주의·임계치 상태는 기존 틸·앰버·빨강 의미를 유지한다. 관리자 기능은 `운영 / 지원 / 경로 / 검토 / 감사` 72px 레일에 배치하되 접근성 이름은 다섯 탭의 전체 명칭을 보존한다.
- 이유: 사용자가 승인한 밝은 하늘색 정체성을 반영하면서 흰색 전경의 낮은 명암비를 피하고, 강한 브랜드색과 운영 상태색의 의미를 분리하기 위해서다. 좁은 레일은 작업면을 넓히되 기존 일일 운영과 감사 기능을 축소하지 않는다.
- 기각한 대안: `#00BFFF` 위 흰색 글자, 네이비 헤더 유지, 하늘색을 안정 상태색으로 재사용, 디자인 시안처럼 일일 운영을 제외한 네 개 탭만 표시.
