...
- 날짜: 2026-08-01
- 상태: Approved
- 결정: 모달 머리말의 `합성 Demo · 지원안 검토`와 후보 영역의 `안전한 후보만 비교`를 삭제한다. 제목은 `{기사명} 기사`, 보조 정보는 `{구역} | 배송 {완료}/{전체}`, 후보 제목은 `지원 선택`, 개수는 `5개`로 표시한다. 후보 카드는 조치명, 조정 후 상태와 배송 시간만 기본 표시하고 추천 또는 차단 이유만 짧게 남긴다. `선택한 조치`는 삭제하고 오른쪽 제목을 `선택 사항`, 전후 라벨을 `현재 → 조정 후`로 바꾼다. 중복되는 `선택` 행과 별도 보전 가정 상자는 제거하고 `배송 시간 / 수신 기사 기준 / 배송 보전`만 표시한다. 초기 확인 상태는 `기사 확인 전 · 현재 계획 유지`, 바닥 출처는 `합성 데모 · 실제 지급·정산 미연동` 한 줄로 통합한다.
