...
- 결정: `/dashboard-demo`는 62px 헤더, 214px 기사 스트립, 가변 지도와 384px 지원 검토 패널의 한 화면으로 구성한다. 기사 카드는 흰색·1px 중립 경계·16px 반경·flat shadow를 기본으로 하며, 54px 합성 프로필, 상태점·상태 pill·약 29px 안전여유 숫자를 함께 사용한다. 긴급 카드만 옅은 빨강 tint를 허용하고 채도 높은 전체 상태색 면은 금지한다. 필터는 비해당 카드를 제거하지 않고 38%로 흐리게 한다. 지도는 24px semantic 원형 마커와 선택 파란 링을 사용한다. 우측 패널은 Safety Margin Track, 45 미만 9명의 지원 시급성 번호·안전여유·ETA, 비성과평가 안내 배너를 포함한다.
