...
- 날짜: 2026-08-12
- 결정: 기사 카드의 Budget은 소수 첫째 자리 숫자로만 표시하고, 30 미만 값이 반올림으로 `30.0`이 되지 않도록 해당 경계 아래에서 보수적으로 절삭한다. ASOS `관측 반영하고 예측`은 선택한 기준 상황을 유지한 채 강수·시정 입력과 결정론적 예상 최저·안전한계 시점·추천의 전후를 같은 화면에서 즉시 다시 계산해 표시한다.
