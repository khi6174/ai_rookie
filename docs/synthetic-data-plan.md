...
- 25개 결정론적 상위 운영 레코드에서 배송 작업표·근무표·배송지 경로표·사고예방 안전보고서 4종을 생성해 비정형 한국어 문서 100개 목표를 충족했다.
- parent record 단위로 development 60·validation 20·frozen-test 20문서를 분할했다.
- Schema·참조·시간·개인정보·의미 일치·안전 책임 경계 위반과 exact duplicate는 모두 0건이다.
- 5개 안전보고서에는 명시적으로 비신뢰 처리된 prompt-injection 문장을 포함한다.
- 이는 규칙 기반 합성 기준선이며 A.X·K-EXAONE 배치 생성, Upstage Live Parse·Extract 또는 자체 모델 학습 완료를 뜻하지 않는다.
