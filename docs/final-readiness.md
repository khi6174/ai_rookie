...
이 판정은 합성 Demo의 기능·재현성 준비 완료를 뜻한다. 이후 G3-B의 설치 가능한 정적 app shell, 승인 Demo 계획 캐시, ADR-043·044의 Kakao Maps 2D 표시 어댑터, ADR-045의 결정론적 Demo 이동, ADR-046의 Fallback 2D 부하 예산과 ADR-047의 활성 decision 선택형 Demo 2.5D가 추가됐지만, 실제 사고감소, 현장 안전성, 실제 기사 데이터 처리, 실제 인증·위치·푸시·TMS와 운영 배포 준비 완료를 뜻하지 않는다.

## 2. 확정된 최종준비 범위

- 승인된 G5-A까지 현재 폐루프를 동결하고 이후에는 재현성·오류·접근성·제출 증거를 우선한다.
...
- 실제 인증·Live 위치·푸시·서버 동기화, 주소·길찾기·실제 WebGL 3D·DEM·건물과 추가 외부 의존성은 별도 승인 대상으로 유지한다.
