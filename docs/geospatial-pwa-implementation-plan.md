...
현재 공개 Demo는 단일 합성 시나리오, schematic `Fallback map`, 역할 전환형 반응형 모바일 웹이다. 이 상태는 최종 디자인이 아니지만, 실지도·실시간 위치·설치형 PWA가 실제 계약과 테스트를 통과하기 전까지 정직한 기준선으로 유지한다.
...
| 6 | G3 기사 PWA 구현 | 권한·캐시 범위 승인 후 진행 |
...
- 설치형 PWA의 인증 방식, 오프라인 범위와 푸시 알림 범위
