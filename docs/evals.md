...
| Playwright 폐루프·접근성·PWA E2E | `pnpm run test:e2e` | 15/15 통과, 기존 폐루프·reset·키보드·4개 해상도·지도 Fallback에 실제 offline reload·승인 계획 읽기 전용·정확 만료·설치 경계 추가 |
...
| G3-B 설치·오프라인 경계 | `pnpm test`, `pnpm run test:e2e` | manifest·아이콘·versioned app shell, 승인 Demo 계획 30분 TTL·정확 만료·손상 격리, 실제 offline reload와 읽기 전용 계획, 만료 최신 승격 0건, 오프라인 응답 성공 0건; Vitest 205/205·Playwright 15/15 |
