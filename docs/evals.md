...
G4-A는 24명·7 frame·5초 cadence와 고정 SHA-256을 계약 테스트로 검증한다. CURRENT 기사만 이동하고 stale·offline 위치 JSON은 frame 전체에서 변하지 않아야 한다. 연결 끊김 두 frame 뒤 새 CURRENT 관측으로만 복구해야 하며 Live 관측 혼합과 깨진 cadence는 100% 거부한다. Playwright는 권역 선택 후 현재 마커 이동·stale 마커 정지, 재생·일시정지·초기화를 확인하고 기존 지도 pan·오류 Fallback·폐루프를 함께 재검증한다. G4-B의 대규모 성능 예산은 아직 통과로 기록하지 않는다.
...
| Playwright 폐루프·접근성·PWA E2E | `pnpm run test:e2e` | 16/16 통과, 기존 폐루프·reset·키보드·4개 해상도·지도 Fallback·bounded drag/keyboard pan에 실제 offline reload·승인 계획 읽기 전용·정확 만료·설치 경계 추가 |
| 서버 clean start 3회 | `pnpm run test:e2e:clean-start` | Vite·브라우저 매회 재기동, 핵심 폐루프 3/3, G3-B 반영 후 최종 재실행 총 21.56초 |
...
| 최신 전체 Vitest | `pnpm test` | 23개 파일, 214/214 통과 |
...
| G3-B 설치·오프라인 경계 | `pnpm test`, `pnpm run test:e2e` | manifest·아이콘·versioned app shell, 승인 Demo 계획 30분 TTL·정확 만료·손상 격리, 실제 offline reload와 읽기 전용 계획, 만료 최신 승격 0건, 오프라인 응답 성공 0건; Vitest 214/214·Playwright 16/16 |
| G4-A 결정론적 Demo 이동 | `pnpm test`, `pnpm run test:e2e` | 24명·7 frame·5초 cadence·고정 SHA-256, CURRENT만 이동, stale·offline 정지, 합성 연결 끊김·복구, Live 혼합 거부, 재생·일시정지·단계·초기화; 기존 16개 E2E 안에서 회귀 검증 |
