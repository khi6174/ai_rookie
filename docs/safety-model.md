...
ADR-152의 합성 운행 frame은 이 초기상태 공식을 우회하지 않는다. 각 tick의 배송 완료·남은 작업량·연속근무·검증된 휴식 입력을 strict 운영 패키지로 다시 만든 뒤 같은 v1 엔진을 실행한다. 화면은 Budget을 시간에 따라 임의 감산하거나 행동 라벨에서 추정하지 않는다. 휴식 완료 뒤 연속근무가 재설정될 때만 기존 Recovery·초기상태 규칙에 따른 개선이 허용되며, 경로 좌표 자체는 Safety 입력이 아니다.
