...
- 관리자 화면의 `decision` scope에서만 `경사 근거 자세히 보기 · Demo 2.5D` 버튼을 제공한다.
...
1. **G5-A 구현 완료:** 활성 지원 decision에만 `경사 근거 자세히 보기 · Demo 2.5D`를 제공한다.
...
Round 1은 기술 성능과 무관하게 사람 이해에서 실패했다. 이에 2D를 더 복잡하게 만들거나 2.5D 입체감을 강화하지 않고, 관리자 오른쪽 decision 면을 먼저 단순화했다.

- 첫 제목은 상태 설명이 아니라 `10분 휴식 + 배송 8건 이관을 진행할까요?`라는 결정 질문이다.
- 경로 순서는 `현재 → 휴식 → 휴식 뒤 경사 노출 → 17번째 배송지 전 지원`으로 고정한다.
- 원 기사와 수신 기사의 뜻, 작업량 변화와 안전여유 전후를 같은 시야에서 비교한다.
- `Demo 2.5D`는 `경사 근거 자세히 보기` 보조 상세이며 기본값은 계속 2D다.
...
### 9.3 Round 3 결과와 Round 4 역할 언어

Round 3도 전체 strict trial 0/6으로 `DO_NOT_PROMOTE`였다. 경사 구간은 6/6이었지만 2D의 시간·배송지와 2.5D의 양측 영향·휴식 선행 순서가 함께 전달되지 않았다. 따라서 2.5D는 계속 보조 상세로 유지한다.

Round 4는 첫 결론에 `52분 후·17번째 배송지 전·10분 휴식·8건 이관`을 함께 표시하고, 사용자 화면의 역할명을 `지원받는 기사`와 `배송을 나눠 맡는 기사`로 풀어 쓴다. 내부 `SOURCE`·`RECIPIENT`, Safety 수치와 정답표는 변경하지 않는다. Round 1·2·3 자극과 결과는 그대로 보존한다.
