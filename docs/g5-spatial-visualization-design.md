...
1. **현재 단계 — 설계 Draft:** 대표 장면, 계약, UX와 Gate 검토
2. **G5-A 구현 승인 필요:** 선택지 A의 관리자 decision-scope 2.5D prototype
3. **G5-A 검증:** 계약·성능·접근성·2D 의미 일치와 사람 이해 확인
4. **공개 Demo 반영 승인 필요:** 기본 2D 유지, 선택형 2.5D 공개 여부 결정
5. **P2 재검토:** 선택지 B 지형 overlay 또는 선택지 C 실제 WebGL 3D

## 10. 이번 승인에서 확정해야 할 항목

- 대표 장면을 `선택 decision의 경사·휴식·예상 초과 설명`으로 고정할지
- 관리자 전용·decision scope·사용자 선택형으로 제한할지
- 기본 2D와 기사 compact map을 변경하지 않을지
- 우선 선택지를 새 의존성이 없는 공급자 독립 2.5D로 정할지
- 합성 고도·경사 fixture의 구체값은 구현 전 계약 테스트와 함께 승인할지
...
- 이 문서는 구현 없이 독립적으로 장면, 데이터, UI, 기술 선택, 위험과 Gate를 설명한다.
- 기존 2D·Fallback·기사 권리·Safety·국내 AI 경계를 약화하지 않는다.
- 현재 기술로 확인되지 않은 Kakao 기능을 3D로 주장하지 않는다.
- 사용자가 10장의 확정 항목을 승인하기 전 기능 코드를 변경하지 않는다.
