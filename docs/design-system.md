...
- 설치형 PWA의 manifest, service worker, 오프라인 캐시와 푸시 권한
- 실제 인증, 별도 기사 세션과 권한 모델
- 실제 지도 공급자 선택, 지도 SDK, TMS·알림·날씨·위치 스트림 실연동
...
이 단계에서는 국내 AI 트랙 범위와 라이선스 근거가 없는 새 이미지·아이콘을 제품 산출물에 넣지 않았다. CSS route, 색·모양·텍스트와 구조화 목록만 사용하며 실제 지도·현재 위치·설치형 PWA처럼 표현하지 않는다.

관리자 지도는 `design-v2.0.0`의 G2 범위를, 기사 화면은 G3-A 시각 구조를 반영했지만 전체 완성 화면은 아니다. 공급자 독립 2D 지도, 지도 오류 목록 Fallback과 접근성 E2E는 구현됐으나 실제 지도 공급자·Live 위치·설치·오프라인 캐시는 미구현이며, 기사 화면은 여전히 반응형 모바일 웹과 compact Fallback map이다. 이 합성 fixture 경계는 후속 지도·PWA 단계가 실제 검증되기 전까지 정직하게 유지한다.
...
최종 목표는 설치 가능한 현장형 기사 PWA다. 현재 공개 데모는 실제 인증·오프라인·푸시를 주장하지 않는 역할 전환형 반응형 모바일 웹이며, 전환 전까지 이 상태를 명시한다. 두 단계 모두 하단 주요 탭은 `운행 / 안전지원 / 내 정보` 세 개로 고정하고 세부 상태를 별도 탭으로 늘리지 않는다.
...
- 성능 수용기준은 별도 architecture·eval 승인에서 기사 수, 갱신주기, 프레임과 지연 예산으로 고정한다.
...
- 설치형 PWA와 실지도는 최종 디자인 목표로 승인됐지만 데이터·개인정보·architecture·공급자 계약 승인과 검증 전에는 현재 데모가 구현된 것처럼 암시하지 않는다.
...
- 아이콘 라이브러리 선택과 라이선스
- 실제 지도 공급자와 지도 스타일
- 2D·2.5D·3D를 제공할 지도 SDK의 성능·접근성·국내 서비스·라이선스 비교
- 실시간 위치의 공급자, 수집주기, 정확도, 보존기간과 권한 모델
- 지역·허브·기사 수에 따른 군집·렌더링 성능 예산
- 설치형 PWA의 오프라인 범위, 캐시 만료, 푸시 권한과 인증 세션
...
위 미결사항은 최종 디자인 방향을 다시 여는 항목이 아니라 구현 수단과 계약을 선택하기 위한 Gate다. 새 지도 공급자, 위치 수집, 설치형 PWA, 실제 인증과 외부 의존성은 관련 승인 문서와 ADR을 갱신하고 사용자 승인을 받은 뒤 도입한다.
