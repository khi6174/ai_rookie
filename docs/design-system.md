...
Shadow Live 준비 화면은 연결 계약 확인용 보조 경로로 유지한다. 공개 관리자의 주 경험은 지도 안에서 자동 재생되는 합성 운행이며 헤더에 `합성 운행 중 · 실제 TMS 아님`을 한 번 지속 표시한다. 지도 툴바에는 현재 가속 시각, `운행·배송·안전여유 반영 중`, 일시정지와 처음부터 제어를 둔다. 선택 기사 경로는 Kakao와 Fallback 모두 도로 polyline으로 표시하고, 기사 카드와 선택 strip에는 `다음 배송지로 이동 / 배송지 정차·전달 / 안전 휴식 중 / 도로 지연 확인` 중 현재 행동을 짧게 표시한다. 별도 재생 화면으로 이동하는 링크는 주요 헤더에서 제거한다.
