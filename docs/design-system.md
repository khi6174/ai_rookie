...
- 기사 마커는 현재 초를 입력으로 한 합성 위치 시뮬레이션을 1초마다 표시한다. 각 마커는 강남권 육지의 미리 지정한 짧은 도로 구간 안에서만 왕복한다. 지도 툴바에 `위치 시뮬레이션`을 짧게 남기며 실제 GPS·TMS·서버 위치 스트림이나 Live 데이터로 표현하지 않는다.
...
- 지도에는 `위치 시뮬레이션 · 현재 시각`, 상태별 개수 범례와 권역별 합성 이관 여력을 제공한다. 합성 마커는 1초마다 움직이고 겹치는 위치는 묶음 수와 대표 밴드를 함께 표시한다.
...
- 20명 사진은 동일한 조명과 구도의 합성 인물 sprite 한 장을 사용한다. 위치 데이터는 지도 툴바의 `위치 시뮬레이션` 표기로 실제 GPS와 구분한다. 사진 주변 세부 용어는 ADR-115의 사용자 선택 문구를 따른다.
