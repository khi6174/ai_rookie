...
2026-07-20 G2-C에서 공급자 독립 합성 지도에 bounded pan을 추가했다. 관리자는 빈 지도 영역을 포인터로 끌거나 지도에 포커스한 뒤 방향키로 이동할 수 있고, `Home`·`0` 또는 `지도 중심 복원`으로 원위치한다. 이동 범위는 화면 안의 Demo 맥락을 잃지 않도록 가로 160px·세로 110px로 제한하며, 마커·지원 decision 클릭과 지도 위 고정 안내는 이동 surface에서 분리한다. 이 기능은 합성 위치를 움직이거나 실제 지도 공급자 동작을 가장하지 않는다.

기사 운행의 지원 계획 요약은 큰 경고성 문장 하나를 전면에 두지 않는다. `지원 계획 상태 → Safe-until 시각 → 현재·다음 조치·지원 시점`의 세 단계로 구성하고, 핵심 시각은 16:20과 약 52분을 함께 보여준다. 상세 원인은 짧은 보조문으로 낮추며 동일한 정보가 아래 경로 지도와 중복되지 않도록 한다.
...
관리자 지도는 `design-v2.0.0`의 G2 범위와 bounded pointer·keyboard pan을, 기사 화면은 G3-A 시각 구조와 G3-B 설치·오프라인 경계를 반영했지만 전체 완성 화면은 아니다. 공급자 독립 2D 지도, 지도 오류 목록 Fallback, 설치 가능한 app shell과 만료 캐시는 구현됐으나 실제 지도 공급자·Live 위치·인증·푸시·서버 동기화는 미구현이다. compact map은 계속 `Fallback map`으로 표시한다.
