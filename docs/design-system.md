...
- 연속작업·안전여유·분담 기사 여유·숙련도와 기상·경로 강도는 range slider를 사용한다. 상태어를 우측에 먼저 표시하고 정확한 값·단위는 아래 보조값으로 유지한다.
...
- 1-1에서 날짜·시간을 선택하면 관측 강수·시정을 1-2에 즉시 설정하고 예상 최저·안전한계 시점·추천의 전후 결과를 1-2 상단에 보여준다. 별도 적용 버튼은 두지 않으며, 선택 시간과 관측값 카드는 두 열에서 남은 폭을 채운다. 강수·시정 slider에는 `관측 적용됨`을 표시하고 사용자가 값을 다시 조정하면 제거한다. 관측 기온은 체감온도로 임의 변환하지 않았음을 함께 명시한다.
