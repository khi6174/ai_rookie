...
- 대회 제출용 주요 화면에는 `Demo`, `합성`, `실제 아님`, `Fallback`, `Live` 같은 개발·검증 용어를 반복하지 않는다. 대신 각 주요 화면 상단에 작은 `시연 데이터` 표시 하나만 유지한다. 오류·오프라인·연결 상태는 `기본 지도`, `연결 확인 중`, `준비 중`처럼 사용자가 행동을 이해할 수 있는 말로 전달하고, 상세 출처 메타데이터는 내부 계약과 검증 자료에 유지한다.
- 실제 한국어 길이로 설계·검증한다.
