...
- 헤더는 왼쪽 `SR` 칩과 워드마크, 중앙 `Safety Control Tower`, 오른쪽 `합성 Demo / Live 0명 / 운영 관리자 / 시각`을 같은 줄에 표시한다.
- 기사 현황 레일의 왼쪽에는 `안전여유 낮은 순 · 지원 시급성`, 오른쪽에는 `전체·위험신호·지원·주의·안정` 필터와 이동 버튼을 같은 줄에 둔다. 필터는 카드를 제거하지 않고 비해당 카드의 투명도를 약 38%로 낮춘다.
- 카드에는 64px 합성 프로필 사진·이름·지정구역·안전여유를 표시한다. 1280×720 압축 레이아웃에서도 사진은 56px를 유지한다. 흰색, 1px 중립 경계, 16px 반경과 `--sh-flat`을 기본으로 하며 한쪽 강조선은 사용하지 않는다. 긴급 카드는 옅은 빨강 tint만 허용한다.
- 카드 상태는 아바타 우하단 상태점, 상태명 pill, 안전여유 숫자색을 함께 사용한다. 숫자는 약 29px이며 선택 카드는 상태색의 사방 2px 링을 사용한다. 빨강은 30 미만 긴급, 앰버는 지원·주의, 그린은 안정에 사용한다.
- 지도에는 `위치 기준 · 시각`, `Demo overlay`, 상태별 개수 범례와 상단 전역 `합성 Demo`, `Live 0명`을 제공한다. 지도 안에서는 `합성 위치` 문구를 반복하지 않는다. 마커는 움직이지 않고 겹치는 위치는 묶음 수와 그 안의 최저 Safety Budget을 함께 표시한다.
...
- 기사 앱의 명시적 위험 신호는 긴급 카드의 옅은 빨강 tint, 빨강 숫자와 `기사앱 위험 신호` 문구로 표시한다. 채도 높은 전체 빨강 면은 사용하지 않으며 동적 `위험신호` 필터와 건수를 갱신한다.
- 실제 지도와 Fallback 지도의 개별 기사 마커는 24px 상태별 semantic 원형, 흰 테두리와 중앙 비색상 점으로 구성한다. 선택은 파란 외곽선으로 표시하고 접근성 이름에는 기사명·상태·안전여유를 제공한다.
- 지도 좌하단에는 선택 기사의 상태, 이름, 구역, 안전여유와 배송 진행을 요약한 칩을 고정한다.
- 우측 `지원 검토` 패널은 선택 기사, 안전여유·안전한계 2칸, 0–100 Safety Margin Track과 `30 한계 / 45 기준 / 60` 틱, `지원안 검토` 행동을 제공한다. 행동은 같은 화면의 모달을 열어 후보 비교 → 기사 확인·수정·거절 → 관리자 승인 → 계획·ETA·고객안내 적용을 끝낸다.
- `지원 기준 45 미만` 목록은 9명이며 `지원 시급성 번호·이름·구역·안전여유·ETA`를 표시한다. 번호는 성과 순위가 아니며 `순위는 지원 시급성이며 기사 성과·평가가 아닙니다. 동의 전에는 현재 계획이 바뀌지 않습니다.` 안내를 파란 tint 배너로 고정한다.
...
- 개입 모달은 흰색 평면, 사방이 같은 경계와 어두운 배경 오버레이를 사용한다. 후보 카드는 그림자 없이 전체 파란 경계와 옅은 파란 배경으로 선택을 표시하고, `Escape`, 배경 클릭, 닫기 버튼, 포커스 순환·복원을 지원한다.
