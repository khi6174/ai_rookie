...
관리자 지도는 `design-v2.0.0`의 G2 범위를, 기사 화면은 G3-A 시각 구조와 G3-B 설치·오프라인 경계를 반영했지만 전체 완성 화면은 아니다. 공급자 독립 2D 지도, 지도 오류 목록 Fallback, 설치 가능한 app shell과 만료 캐시는 구현됐으나 실제 지도 공급자·Live 위치·인증·푸시·서버 동기화는 미구현이다. compact map은 계속 `Fallback map`으로 표시한다.
