...
- 기사 카드, 지도 선택 스트립과 지원 큐에서는 Safety Budget을 `안전지원 점수`로 표시한다. 점수가 낮은 순서로 배치하되 기사 성과·등급·경쟁 순위가 아니라 현재 계획의 안전지원 필요도를 부각하는 값임을 짧은 `순위 아님` 표기로 알린다.
...
- 기사 마커는 현재 초를 입력으로 한 bounded 합성 위치 시뮬레이션을 1초마다 표시한다. 지도 툴바에 `위치 시뮬레이션`을 짧게 남기며 실제 GPS·TMS·서버 위치 스트림이나 Live 데이터로 표현하지 않는다.
...
- 기사 현황 레일의 왼쪽에는 `안전지원 점수 낮은 순 · 순위 아님`, 오른쪽에는 `전체·위험신호·지원·주의·정상` 필터와 이동 버튼을 같은 줄에 둔다. 필터는 카드를 제거하지 않고 비해당 카드의 투명도를 약 38%로 낮춘다.
- 카드에는 64px 합성 프로필 사진·이름·지정구역·밴드와 약 29px 안전지원 점수를 표시한다. 1280×720 압축 레이아웃에서도 사진은 56px를 유지한다. 흰색, 1px 중립 경계, 16px 반경과 `--sh-flat`을 기본으로 하며 한쪽 강조선은 사용하지 않는다. 한계 초과 카드는 옅은 빨강 tint만 허용한다.
- 카드 상태는 아바타 우하단 상태점, 상태명 pill, 안전지원 점수와 짧은 조치 시각을 함께 사용한다. 선택 카드는 상태색의 사방 2px 링을 사용한다. 빨강은 30 미만 한계 초과, 앰버는 지원 필요·주의, 그린은 정상에 사용한다.
...
- 지도 좌하단에는 선택 기사의 상태, 이름, 구역, 안전지원 점수, 조치 시각과 배송 진행을 요약한 칩을 고정한다.
- 우측 `지원 검토` 패널은 선택 기사, 현재 밴드·조치 시각 2칸, 관리자 엔진 상세용 0–100 Safety Margin Track, 합성 이관 여력과 `지원안 검토` 행동을 제공한다.
- 지원안 모달은 후보 비교와 이관 여력 부족 전환, 이관 불가 시 대체 계층, Demo 보전 가정을 제공한다. 물량이관은 원 기사 확인 → 수신 기사 확인 → 관리자 승인 → 계획·ETA·고객안내 적용 순서를 유지한다.
- `지원 기준 45 미만` 목록은 9명이며 이름·구역·안전지원 점수·조치 시각을 표시하고 개인 순번은 표시하지 않는다. 패널 최하단의 장문 안내 배너는 사용하지 않는다.
...
- 20명 사진은 동일한 조명과 구도의 합성 인물 sprite 한 장을 사용한다. 위치 데이터는 지도 툴바의 `위치 시뮬레이션` 표기로 실제 GPS와 구분하며 사진 주변 세부 용어는 사용자와 함께 후속 확정한다.
