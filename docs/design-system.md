...
- 기사 앱에서 명시적인 현재 위험 신호가 수신된 경우에만 해당 기사 카드 전체를 빨간색으로 전환한다. 낮은 Safety Budget만으로 카드 전체를 빨갛게 만들지 않는다. 빨간 카드에는 흰색 `기사앱 위험 신호` 문구와 안전여유 숫자·상태어를 함께 표시하며, 그림자나 한쪽 강조선을 사용하지 않는다.
...
- 프로필 카드는 상단 왼쪽에 약 84~90px의 정사각형 사진, 오른쪽에 이름·지정구역을 밀착 배치하고, 하단에는 카드 폭 전체를 쓰는 46px 이상의 안전여유 숫자와 상태어를 `38.9 지원`처럼 한 줄로 묶는다. 별도의 하단 박스처럼 보이지 않도록 상태별 옅은 배경색을 카드 전체에 적용하고 내부 패딩은 6~7px, 상·하단 간격은 4px 수준으로 제한한다. `안전여유` 라벨과 숫자 행은 위에서부터 연속 배치해 간격을 0~2px로 유지한다. 이모지·기호 없이 `긴급·지원·주의·안정` 상태어를 제공하고 기사 성과 점수로 부르지 않는다. 소셜 프로필의 팔로워·인증·소개문·Follow 행동은 가져오지 않는다.
