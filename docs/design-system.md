...
- 우측 `안전 지원` 패널은 `선택 기사`, 지원 인원 `9/20`, `현재 상태·지원 시점` 2칸, 관리자 엔진 상세용 0–100 Safety Margin Track, `배송 분담 · 강남` 집계와 `지원 검토` 행동을 제공한다. 최초 선택 기사는 `위험`, `지금`, `기사 지원 요청`, `배송 14/31 완료`로 읽힌다.
- 지원 검토 모달은 `기사명 / 구역 | 배송 진행`, `지원 선택 / 5개`, 짧은 후보 결과와 `선택 사항`을 우선한다. 후보 카드는 조치명·조정 후 상태·배송 시간만 기본 표시하고 추천 또는 차단 이유만 한 줄로 덧붙인다. 배송 분담 정보는 후보 위에서 제거하고 오른쪽 선택 사항 아래에 배치한다. `가능 기사 4명 / 최대 11건`과 현재 선택의 `분담 없음`, `현재 선택 N건 / 가능·불가`만 표시하며 백분율과 전체 필요량은 사용하지 않는다. `분담 불가 상황 보기` 전환과 `분담 불가 시: 휴식 → 순서 변경 → 시간 재약정` 안내는 유지한다. 실제 지급·정산 미연동 표기는 바닥의 단일 합성 데모 문구로 합친다. 물량이관은 원 기사 확인 → 수신 기사 확인 → 관리자 승인 → 계획·ETA·고객안내 적용 순서를 유지한다.
