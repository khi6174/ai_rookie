...
- 독립 검토 자극은 평가 대상 공개 화면과 같은 경로·컴포넌트·viewport를 사용한다. 기술 검증용 보조 화면을 현재 공개 UI의 대리 자극으로 사용하지 않는다.
...
- 운행 탭의 현재 위치 지도는 배송 구역 카드 안에서 진행·경로와 함께 제공하고, 위치 권한은 `내 위치 표시`라는 44px 이상의 명시적 버튼으로 요청
- 지도 위 본인 마커는 흰색·블루 계열의 택배 탑차 상단만 보이는 2D 하향 시점 아이콘과 `내 위치` 꼬리표를 함께 사용하며 차량 방향이나 움직임을 색만으로 전달하지 않음
- 본인 마커는 지도 확대 시 커지고 축소 시 작아지며 지도 폭까지 반영해 24~108px 안에서 조절함. level 1~3은 탑차와 꼬리표, 4~5는 축소 탑차, 6 이상은 상태 점으로 바꾸며 기사·관제 지도에 같은 단계를 사용함. 연속 위치는 짧고 부드럽게 이어 표시하되 움직임 줄이기 설정에서는 즉시 이동함
- 기기 위치·경로 위치·확인 중·갱신 지연·권한 없음·사용 불가를 짧은 텍스트로 구분하고, 권한 거절 상태를 경고색이나 불이익으로 표현하지 않음
- Kakao 지도 실패 시 같은 카드 크기의 schematic 경로와 동일한 택배차 마커를 유지해 배송 정보와 하단 탐색이 밀리지 않게 함
...
2026-08-10 공개 관제 지도는 사용자가 직접 조정한 확대·축소와 이동을 1초 운행·5초 Safety 갱신으로 덮어쓰지 않는다. 카메라 자동 맞춤은 최초 로드, 사용자가 누른 `배송구역 확대`·`전체 보기`, 확대 상태에서의 기사 변경에만 실행한다. 기사마다 자기 도로 경로의 현재 좌표를 독립 anchor로 유지하며, 화면에서 마커의 터치 영역이 충돌할 때만 최대 3열의 결정론적 표시 오프셋을 더한다. 허브 평균점이나 공통 중심으로 기사들을 함께 이동시키지 않는다. Kakao와 지도 대체 화면이 같은 규칙을 사용한다.
...
- 기사 카드, 지원 큐, 선택 카드의 상태·선택 표현에 왼쪽 또는 오른쪽 한쪽만 강조하는 `border`, `inset box-shadow`, 색상 띠를 사용하지 않는다. 선택은 사방의 동일한 경계 또는 배경색으로 표현한다.

### 4.6 그림자

- 신규·변경 화면에는 `box-shadow`, `text-shadow`, CSS `filter: drop-shadow()`를 사용하지 않는다.
- 카드·패널 상태를 왼쪽이나 오른쪽의 두꺼운 `border` 또는 색상 띠로 강조하지 않는다. 사방이 같은 굵기와 색의 경계를 사용한다.
...
- 텍스트 카드의 좌·우 측면에 상태색 띠나 선택용 inset 그림자를 붙이지 않는다. 카드 구조는 중립 경계로 유지하고 상태는 badge 안에서 표현한다.
...
1. 현재 배송 구역의 대표 사진과 `시·구·합성 아파트명`
2. 현재 계획 기준 배송률·완료/전체 건수·예상 완료 시각
3. 내 현재 안전 지원 점수와 위험 예상까지 남은 분·배송지
4. 현재 위치, 진행 방향과 다음 배송지를 보여주는 지도 작업면
5. 오늘의 안전배송, 배송 진행과 Safe-until
6. 현재 위치·휴식 지점·다음 배송지를 잇는 compact route
7. 적용된 다음 계획 또는 검토할 지원안
8. 안전지원 검토 버튼
9. `합성 예시 / 실제 신고 아님`을 함께 표시하는 별도 `응급 상황 감지 예시` 버튼

운행 탭의 첫 카드는 `서울시 용산구 한빛아파트`라는 합성 구역명과 서울 주거지역 대표 사진을 사용한다. 사진은 실제 배송지나 현재 위치 증거가 아니며 대체 텍스트와 에셋 출처를 기록한다. 배송률은 기존 Demo 배송 진행 `14/31`에서 계산하고, 예상 완료 시각은 현재 workload의 `projectedEndAt`을 사용한다. 초기 대표 값은 `14/31 · 45%`, 현재 점수 `54.7`, `52분 뒤 · 17번째 배송지 전`이다.
...
`응급 상황 전송`은 실제 센서 감지나 응급 신고 기능이 아니다. 버튼을 누르면 현재 합성 기사 ID의 15분짜리 Demo 위험 신호만 공유 운영 저장소를 거쳐 열린 관제 화면에 전달한다. 실제 위치·센서·연락처는 보내지 않으며, 빨강은 이 위험 행동에만 사용한다.
...
### 8.13 단일 관제 데모의 시간·지원·검증 표현

- 기사 카드, 지도 선택 스트립과 지원 큐에서는 Safety Budget을 `안전 지원 점수`로 표시한다. 점수가 낮은 순서로 배치하되 개인 순번이나 경쟁형 랭킹 표현은 제공하지 않는다. 화면에서 반복되던 `순위 아님` 문구는 삭제한다.
- 선택 기사 상세에서는 밴드와 `현재 한계 초과`, `한계까지 N분`, `향후 60분 내 초과 예상 없음`을 함께 사용한다.
- 시간은 `BreachPrediction` 또는 명시적인 결정론적 Demo fixture에서만 읽으며 Budget 차이로 역산하지 않는다.
- 관리자 선택 상세의 Safety Margin Track은 엔진 비교 근거로 유지하되 성과 점수처럼 단독 강조하지 않는다.
- 지원 큐는 안전지원 점수가 낮은 순으로 보여주되 개인 순번과 경쟁형 랭킹 표현은 제공하지 않는다.
- 이관 여력은 합성 집계, 필요 물량, 동일 위험 영향과 함께 표시한다. 대체 처방 계층은 추천 순위가 아니라 이관 불가 시 남는 안전한 선택지를 설명한다.
- 단일 관제 헤더와 작업면에는 별도 검증 오버레이를 두지 않는다. 미측정 성능과 잠정 운영 기준은 승인 문서와 상세 검증 화면의 근거로 유지하며 실제 검증 완료 효과로 표현하지 않는다.
- 신뢰도는 입력 품질이며 통계적으로 검증된 예측구간이 없으면 `±` 추정 폭을 붙이지 않는다.
- 정산 보전은 실제 계약·지급 연동 전까지 `Demo 보전 가정 · 실제 지급 연동 없음`으로만 표현한다.
- 물량이관은 원 기사와 수신 기사 모두 같은 수준의 동의·수정 요청·거절 행동을 제공하고, 두 동의 뒤에만 관리자 최종 승인을 허용한다.
...
Shadow Live 준비 화면은 연결 계약 확인용 보조 경로로 유지한다. 공개 관리자의 주 경험은 지도 안에서 계속 갱신되는 합성 운행이며 헤더에 `합성 운행 중 · 실제 TMS 아님`을 한 번 지속 표시한다. 지도 툴바에는 `운행·배송·안전여유 연속 반영 중` 상태와 지도 보기 제어만 두고 `일시정지`, `계속`, `처음부터`를 노출하지 않는다. 선택 기사 경로는 Kakao와 Fallback 모두 도로 polyline으로 표시하고, 기사 카드와 선택 strip에는 `다음 배송지로 이동 / 배송지 정차·전달 / 안전 휴식 중 / 도로 지연 확인` 중 현재 행동을 짧게 표시한다. 별도 재생 화면으로 이동하는 링크는 주요 헤더에서 제거한다.

같은 권역 기사에게 하나의 굵은 공통 경로를 반복하지 않는다. 전체 보기에서는 25개 고유 경로를 낮은 대비로 두고, 선택 기사는 틸 실선과 파란 번호 배송지 최대 4개로 강조한다. `배송구역 확대 / 전체 보기`는 같은 버튼 자리에서 전환하고 focus 상태를 `aria-pressed`와 지도 데이터 속성으로 함께 전달한다. 선택 strip은 변하는 첫 남은 배송지 구역명보다 고정된 `권역 · n코스`를 우선 표시한다.
...
- 공개 기본 경로 `/`는 단일 관제 데모를 표시하며 `/dashboard-demo`는 같은 화면의 호환 경로로 유지한다. 기존 관리자 폐루프 화면은 `/closed-loop-demo`에서 경로만 보존한다.
- 단일 관제 헤더에서는 `폐루프 검증`, `합성 Demo`, `Live 0명`, `검증 상태`, `운영 관리자` 칩을 제거하고 브라우저 현재 시각을 초 단위로 표시한다. `/closed-loop-demo` 경로 자체는 삭제하지 않는다.
- 기사 마커는 현재 초를 입력으로 한 가상 위치를 1초마다 갱신하되 도로 구간 한쪽 끝까지 약 30초에 걸쳐 이동한다. 각 마커는 강남권 육지의 미리 지정한 짧은 도로 구간 안에서만 왕복한다. 지도 툴바에는 `위치 갱신`으로 표시하고 화면 상단의 `시연 데이터` 한 곳에서 출처 경계를 전달한다.
- 배송 분담 후보와 선택 요약에는 `배송 4건 분담 · 안재민 기사`처럼 수신 기사 이름을 함께 표시한다. 기사 확인 요청 뒤에는 `안재민 기사 확인을 기다립니다`, 적용 뒤에는 동일 기사 이름을 포함한 결과를 보여준다.
- 단일 관제 데모는 1920 데스크톱에서 62px 헤더, 기사 현황 스트립, `지도 + 지원 검토` 작업면을 스크롤 없는 한 화면으로 구성한다. 작업면은 16px 간격으로 지도를 가변 주 영역, 지원 검토 패널을 384px 고정 영역으로 둔다.
- 헤더는 왼쪽 `SR` 칩과 워드마크, 중앙 `Safety Control Tower`, 오른쪽 브라우저 현재 시각과 `기사 앱` 버튼을 같은 줄에 표시한다. `기사 앱`은 로그인 단계를 생략한 고정 합성 기사 운행 화면 `/rider-demo`로 이동한다.
- 기사 현황 레일의 왼쪽에는 `안전 지원 점수`, 오른쪽에는 `전체·위험신호·지원·주의·정상` 필터와 이동 버튼을 같은 줄에 둔다. 필터는 카드를 제거하지 않고 비해당 카드의 투명도를 약 38%로 낮춘다.
- 카드에는 64px 합성 프로필 사진·이름·지정구역·밴드와 약 29px 안전 지원 점수를 표시한다. 1280×720 압축 레이아웃에서도 사진은 56px를 유지한다. 흰색, 1px 중립 경계, 16px 반경과 `--sh-flat`을 기본으로 하며 한쪽 강조선은 사용하지 않는다. 한계 초과 카드는 옅은 빨강 tint만 허용한다.
- 카드 상태는 아바타 우하단 상태점, 상태명 pill, 안전 지원 점수와 짧은 지원 시점을 함께 사용한다. 선택 카드는 상태색의 사방 2px 링을 사용한다. 빨강은 30 미만 한계 초과, 앰버는 지원 필요·주의, 그린은 정상에 사용한다.
- 지도에는 `위치 갱신 · 현재 시각`, 상태별 개수 범례와 권역별 이관 여력을 제공한다. 기사 마커는 1초마다 움직이고 겹치는 위치는 묶음 수와 대표 밴드를 함께 표시한다.
- 공개 Demo에 승인된 Kakao Maps JavaScript 키가 있으면 실제 강남 도로 베이스맵 위에 결정론적 Demo 기사 좌표와 강남 허브를 표시한다. 키·도메인·네트워크 오류에서는 `Fallback`을 표기하고 같은 선택 상태를 유지한 채 기존 Fallback 지도로 자동 복귀한다.
- 카드·지도·지원 큐 선택을 동기화한다. 지도나 지원 큐 선택 시 해당 카드가 보이는 위치로 이동한다.
- 기사 앱의 명시적 위험 신호는 한계 초과 카드의 옅은 빨강 tint와 `기사앱 위험 신호` 문구로 표시한다. 초기 합성 신호시각은 15:27이다. 채도 높은 전체 빨강 면은 사용하지 않으며 동적 `위험신호` 필터와 건수를 갱신한다.
- 실제 지도와 Fallback 지도의 개별 기사 마커는 24px 상태별 semantic 원형, 흰 테두리와 중앙 비색상 점으로 구성한다. 선택은 파란 외곽선으로 표시하고 접근성 이름에는 기사명·밴드·조치 시각을 제공한다.
- 지도 좌하단에는 선택 기사의 상태, 이름, 구역, 안전 지원 점수, 지원 시점과 배송 진행을 요약한 칩을 고정한다.
- 우측 `안전 지원` 패널은 `선택 기사`, DB의 현재 지원 대상 비율, `현재 상태·지원 시점` 2칸, 관리자 엔진 상세용 0–100 Safety Margin Track, `배송 분담 · {현재 권역/허브}`와 `수신 가능 {n}명 / 최대 {n}건`, `지원 검토` 행동을 제공한다. 즉시 상태는 `지금`으로 읽힌다. `이관만으로 해소되지 않음`, `순위 아님`, `동일 강우셀 영향 18명`은 표시하지 않는다.
- 지원 검토 모달은 `기사명 / 구역 | 배송 진행`, `지원 선택 / {실제 후보 수}`, 짧은 후보 결과와 `선택 사항`을 우선한다. 후보 카드는 실제 엔진 후보의 조치명·조정 후 상태·배송 시간만 기본 표시하고 추천 또는 차단 이유만 한 줄로 덧붙이며 `/`로 구분한다. 배송 분담 정보는 오른쪽 선택 사항 아래에 배치한다. `가능 기사 {n}명 / 최대 {n}건`과 현재 선택의 `분담 없음`, `현재 선택 N건 / 가능·불가`만 표시하며 백분율과 전체 필요량은 사용하지 않는다. `분담 불가 상황 보기`는 차단 후보를 포함한 현재 엔진 결과를 설명한다. 물량이관은 원 기사 확인 → 수신 기사 확인 → 관리자 승인 → 계획·ETA·고객안내 적용 순서를 유지한다. 관리자는 팝업에서 기사 응답을 대신 선택하지 않는다.
- `안전 지원 점수` 목록은 9명이며 이름·구역·점수·지원 시점을 표시하고 개인 순번과 `순위 아님` 문구는 표시하지 않는다. 패널 최하단의 장문 안내 배너는 사용하지 않는다.
- 색·타이포·반경은 `src/tokens/colors.css`, `typography.css`, `radii.css`가 기존 전역 디자인 토큰을 별칭으로 제공한다. 대시보드 CSS에는 임의 색상 값을 추가하지 않으며 Pretendard Variable과 tabular numbers를 전역 적용한다.
- 허브 위치는 의미가 불명확한 단일 `H` 문자를 사용하지 않는다. 네이비 건물 형태의 CSS 아이콘과 `강남 허브` 텍스트를 흰색 가로형 표식 안에 함께 표시한다.
- 개입 모달은 흰색 평면, 사방이 같은 경계와 반투명 배경 오버레이를 사용한다. 후보 카드는 그림자 없이 전체 파란 경계와 옅은 파란 배경으로 선택을 표시하고, `Escape`, 배경 클릭, 닫기 버튼, 포커스 순환·복원을 지원한다.
- 기본 높이는 헤더 62px, 기사 스트립 214px, 지도 가변이다. 1280×720에서도 헤더 62px을 유지하고 기사 스트립 190px, 카드 128px로 압축한다.
- 20명 사진은 동일한 조명과 구도의 인물 sprite 한 장을 사용한다. 각 기사 이름·구역·배송 진행·도로 구간은 고유 ID와 일대일로 연결하고 출처 표시는 화면 상단 `시연 데이터` 한 곳에만 둔다. 사진 주변 세부 용어는 ADR-115의 사용자 선택 문구를 따른다.
...
- 대회 제출용 주요 화면에는 `Demo`, `합성`, `실제 아님`, `Fallback`, `Live` 같은 개발·검증 용어를 반복하지 않는다. 관리자 관제는 상단의 작은 출처 표시 하나를 유지하되 기사 앱의 운행 화면에서는 `시연 데이터`와 `정차 확인` 배지를 제거한다. 기사 앱의 출처·개인정보 경계는 `내 정보 > 데이터 안내`에서 한 번 확인한다. 오류·오프라인·연결 상태는 `기본 지도`, `연결 확인 중`, `준비 중`처럼 사용자가 행동을 이해할 수 있는 말로 전달한다.
- 기사 `운행` 탭은 기사별 배송 구역, 배송 진행, 남은 배송, 예상 완료와 세 지점 운행 요약만 한 카드에 묶는다. 점수·위험 예상·지원안·응급 감지 예시는 하단 `안전지원` 탭에서 확인한다.
- 실제 한국어 길이로 설계·검증한다.
...
## 26. 과거 공개 Demo 시각 기준선 동결 — Superseded

이 절의 가산형 변경 제한은 2026-08-07 ADR-146으로 대체됐다. 밝은 운영 색상, 동일 decision 선택 동기화, 사람 동의·승인과 지정 해상도 기준은 계속 유지하지만 기사 카드·지도·지원 큐의 과거 배치와 패널 비율은 더 이상 고정 기준선이 아니다.

### 반드시 유지할 것

- 밝은 페이지, 흰 패널, 블루 운영 행동, 틸 안전 상태, 앰버 대기와 제한적인 빨강의 현재 토큰 체계
- 기사 카드·지도·지원 큐가 같은 기사 선택 상태를 공유하는 한 화면 구조
- 선택 기사에서 지원 검토 모달로 이어지는 폐루프와 사람 동의·승인 순서
- `향후 60분 안에 어떤 지원이 필요한가`에 먼저 답하는 정보 우선순위
- 관리자의 1440×900·1280×720 및 기사의 390×844·360×800 핵심 동작

### 허용되는 가산형 개선

- 기존 AI 설명 블록 안의 공급자·Live/Fallback·검증 상태와 짧은 출처 표시
- 현재 패널 또는 모달 안에서 펼치는 A.X v2 검증 근거와 심사 질의응답용 요약
- 기존 승인 합성 프로필 sprite의 서로 다른 얼굴을 기사 식별에 복구하고, 사진이 없는 기사는 중복 사진 대신 번호형 대체 표시를 사용한다.
- `기사 추가`는 실제 개인정보나 사진 업로드 없이 합성 별칭·기존 허브·거친 권역만 받는 검증 전 등록 요청 dialog로 제공한다. 등록 대기는 활성 기사 수와 Safety 계산을 바꾸지 않는다.
- Loading·Empty·Error·Fallback·Offline 상태, 재시도와 마지막 승인 상태 안내
- 같은 decision ID와 허용된 근거를 사용하는 기사 화면의 정차 후 `왜 이 지원인가` 상세
- 감사 식별자, 마지막 갱신 시각, 키보드·포커스·스크린리더·reduced-motion 보강

### 금지되는 변경

- 공개 루트의 전역 셸, 패널 비율, 핵심 색상과 주 행동 위치를 전면 재설계하지 않는다.
- 지도·기사 카드·지원 큐·지원 검토 폐루프를 별도 화면으로 분산하지 않는다.
- A.X v2 합성 평가 PASS를 현재 Local runtime 활성화나 실제 현장 성과로 표현하지 않는다.
- Safety 수치, 후보 실행 가능성, 추천, 기사 동의·거절 권리와 관리자 승인 권한을 UI가 다시 계산하거나 바꾸지 않는다.
- STT, 실제 음성 저장·전송, Local runtime 활성화는 별도 승인과 계약·개인정보·운영 Gate 전까지 이 기준선 개선에 포함하지 않는다.

가산형 변경도 기존 단위 테스트, 관리자·기사 폐루프 E2E, 지정 해상도와 production build를 통과해야 공개할 수 있다. 기준선 회귀가 생기면 새 기능을 축소하거나 접을 수 있는 보조 영역으로 되돌린다.

## 27. 향후 60분 판단 브리핑형 Control Tower

### 첫 화면 질문과 읽기 순서

관리자 첫 화면은 다음 순서로 읽혀야 한다.

1. 향후 60분 안에 지원 판단이 필요한 기사는 몇 명인가.
2. 지금 선택한 지원받는 기사는 언제·몇 번째 배송지 전에 지원이 필요한가.
3. 현재에서 예상 최저 Safety Budget이 어떻게 변하는가.
4. 휴식과 배송 분담 중 무엇을 먼저 비교하며 위험전가 검사를 통과할 범위는 무엇인가.
5. 영향 기사 확인과 관리자 승인 전에는 현재 계획이 유지되는가.
6. 지도에서 어느 허브·권역·배송 맥락인지 확인한다.
7. 전체 25명 현황은 첫 판단 뒤에 탐색한다.

### 고정 정보위계

- 상단 브리핑: 지원 필요 인원, 합성 운영자료·실제 GPS 아님·승인 전 계획 유지 경계
- 선택 판단: `지원받는 기사`, 지원 시점과 배송 순서, 현재→예상 최저, 비교할 지원, 위험전가 검사 범위
- 적용 순서: `안전한 후보만 비교 → 영향 기사 확인 → 관리자 승인 후 적용`
- 작업영역 왼쪽: 빠른 지원 시점순 목록과 선택 기사 파생 상태
- 작업영역 오른쪽: 선택 기사·허브·배송 맥락을 확인하는 2D 지도
- 하단: 합성 프로필 사진과 예상 최저 Budget을 사용하는 압축형 25명 카드 rail 및 검증 전 기사 등록

### 용어와 표현

- `원 기사`는 화면에서 `지원받는 기사`로 쓴다.
- `수신 기사`는 화면에서 `배송을 나눠 맡는 기사`로 쓴다.
- Safety Budget은 `운영 위험지수` 문맥과 함께 표시하며 사고확률·건강진단·성과평가로 표현하지 않는다.
- 빨강은 현재 한계 초과에만 사용하고 미래 지원·대기 상태는 앰버와 텍스트를 함께 사용한다.
- 합성 지도는 실제 GPS나 현재 정밀 위치로 표현하지 않는다.

### 상호작용과 접근성

- 지원 목록·지도·전체 기사 카드는 같은 기사 선택을 공유한다.
- 주 행동 `지원 검토`는 첫 브리핑에 하나만 둔다. 보조 패널은 같은 위치로 돌아가는 링크를 제공한다.
- 지원 검토 dialog는 후보 목록보다 먼저 지원 시점과 사람 승인 전 계획 불변을 읽는다.
- 관리자 1440×900·1280×720에서 브리핑, 지원 목록, 지도와 전체 기사 rail이 한 viewport 안에 유지돼야 한다.
- 기존 키보드 포커스, modal trap·복원, reduced-motion과 색상 외 상태표현을 유지한다.

### 변경 금지 경계

- UI가 Safety 수치, 후보 실행 가능성, 추천 또는 적용 상태를 다시 계산하지 않는다.
- 기사 동의·수정·거절·이의제기와 관리자 승인 순서를 약화하지 않는다.
- 실제 개인정보·정밀 위치·생체정보·실제 고객 발송을 추가하지 않는다.
- 사람 검토가 통과하기 전에는 새 정보위계의 이해도 개선을 완료 사실로 주장하지 않는다.

## 28. Scenario 날짜·상태형 입력

- 입력은 `1-1 관측 시점 선택 → 1-2 업무 및 안전여유 → 하나의 예상 결과` 순서로 읽힌다. 두 입력 단계는 별도 경계·제목·설명을 사용하되 최종 결과를 서로 다른 예측처럼 나누지 않는다.
- 최근 31일 달력은 실제 요일 열, 44px 날짜 버튼, 선택일의 색상 외 `aria-pressed` 상태를 가진다. 비·고온 점 표시는 보조 정보이며 접근성 이름에 같은 상태를 포함한다.
- 연속작업·현재 안전여유·분담 기사 여유는 range slider를 사용한다. 상태어를 우측에 먼저 표시하고 정확한 값·단위는 아래 보조값으로 유지한다. 남은 배송과 총 근무는 단위가 붙은 숫자 입력이다.
- range thumb는 최소 22px 시각 크기와 44px 이상 조작 행을 사용하고 키보드 화살표 조작·`aria-valuetext`를 제공한다.
- ASOS Live, 확인 중, 활용신청 대기와 오류는 텍스트 badge로 구분한다. 대기·오류에서도 날짜와 임의 조건 조작을 막지 않는다.
- 1-1에서 날짜·시간을 선택하면 관측 강수·시정을 1-2에 즉시 설정하고 예상 최저·안전한계 시점·추천의 전후 결과를 1-2 상단에 보여준다. 별도 적용 버튼은 두지 않으며, 선택 시간과 관측값 카드는 두 열에서 남은 폭을 채운다. 기본 상태는 `ASOS 관측 기반`이다. `우천·경사`, `폭염·계단`, `야간·낯선 권역`처럼 관측·시간·경로 출처를 섞는 복합 프리셋은 노출하지 않는다. 관측 기온은 체감온도로 임의 변환하지 않았음을 함께 명시한다.
- 1-2에는 업무 및 안전여유에 해당하는 다섯 입력만 노출한다. 가상 기상·경로·숙련도 조절은 두지 않으며 경로 조건과 체감온도가 시연 기준계획의 고정값임을 예측 버튼 아래에 짧게 표시한다.
- Budget 소수 첫째 자리 표시는 임계치 아래 값을 반올림해 상위 밴드로 보이게 하지 않는다. 30 미만 경계값은 `29.9`처럼 임계치 아래에서 절삭하며 부등호 문자열을 사용하지 않는다.
- 데스크톱 Scenario 본문은 왼쪽 `01 예측 조건`과 오른쪽 `02 예상 결과` 두 카드가 같은 상단선·높이와 20px 간격을 가진다. `04 현재 자원`은 전체 너비 하단 카드가 아니라 오른쪽 카드의 `03 개입 비교` 바로 아래에 2×2 정보 카드로 배치한다. 모바일에서는 같은 순서를 한 열로 유지한다.
- 관측값을 반영했지만 예상 최저·안전한계 시점·추천이 모두 동일하면 동일한 전후 수치를 세 칸으로 반복하지 않는다. 제목을 `관측 입력`으로 표시하고 `예측 결과 변화 없음`과 현재 위험 구간을 바꾸지 않았다는 이유를 보여준다. 결과가 실제로 달라졌을 때만 세 전후 지표를 표시한다.
- 관제와 기사 앱의 합성 경로는 같은 기사 ID·Kakao Mobility 도로 polyline·공유 progress 시각을 사용한다. 기사 Kakao 지도에도 경로선을 표시하고 마커를 그 선 위에서 이동한다. 기본 카메라는 기존보다 지도 level 1단계 넓게 시작하며 사용자의 확대·축소는 계속 허용한다.
