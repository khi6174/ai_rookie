...
- 입력은 `1-1 관측 시점 선택 → 1-2 시뮬레이션 조건 설정 → 하나의 예상 결과` 순서로 읽힌다. 두 입력 단계는 별도 경계·제목·설명을 사용하되 최종 결과를 서로 다른 예측처럼 나누지 않는다.
- 최근 31일 달력은 실제 요일 열, 44px 날짜 버튼, 선택일의 색상 외 `aria-pressed` 상태를 가진다. 비·고온 점 표시는 보조 정보이며 접근성 이름에 같은 상태를 포함한다.
- 연속작업·안전여유·분담 기사 여유·숙련도와 경로 강도는 range slider를 사용한다. 가상 기상 모드를 켠 경우에만 강수·체감온도·시정 slider를 추가로 보여준다. 상태어를 우측에 먼저 표시하고 정확한 값·단위는 아래 보조값으로 유지한다.
...
- `가상 기상 스트레스 테스트`는 선택 기능이다. 켜기 전에는 기상 slider를 숨기고, 켜면 `사용자 가상 기상` 상태와 함께 강수·체감온도·시정을 조정할 수 있게 한다. 업무·배송 경로 조건은 관측 기상과 별도 영역으로 유지한다.
