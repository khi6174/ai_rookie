...
2026-08-10 공개 관제 지도는 사용자가 직접 조정한 확대·축소와 이동을 1초 운행·5초 Safety 갱신으로 덮어쓰지 않는다. 카메라 자동 맞춤은 최초 로드, 사용자가 누른 `배송구역 확대`·`전체 보기`, 확대 상태에서의 기사 변경에만 실행한다. 기사 위치와 경로 좌표는 바꾸지 않고, 화면에서 마커의 터치 영역이 충돌할 때만 최대 3열의 결정론적 표시 오프셋을 적용한다. Kakao와 지도 대체 화면이 같은 규칙을 사용한다.
