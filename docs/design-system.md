...
- `/dashboard-demo`는 상단 기사 카드, 중단 합성 위치 지도, 하단 향후 60분 시뮬레이션을 스크롤 없는 한 화면으로 구성한다.
- 카드에는 합성 이름·ID·담당 구역·배송 진행·Safety Budget만 남기고 순번, 성과, 원시 생체정보를 표시하지 않는다.
- 지도는 `합성 위치 · 시각`과 `Live 0`을 항상 표시한다. 마커는 움직이지 않고 겹치는 위치는 묶음 수와 그 안의 최저 Safety Budget을 함께 표시한다.
- 하단은 지원 검토가 필요한 기사만 노출한다. 45 미만은 앰버 사선, 30 미만 예상 초과는 빨강 실선, 정상 11명은 `✓ 11`로 압축한다.
- 카드·지도·시뮬레이션 선택을 동기화한다. 지도 선택 시 해당 카드가 보이는 위치로 이동하고 같은 기사 행을 강조한다.
- 기본 높이는 헤더 54px, 카드 164px, 지도 가변, 시뮬레이션 218px이다. 1280×720에서는 헤더 50px, 카드 146px, 시뮬레이션 210px로 축소하되 핵심 상태값은 14px 이상을 유지한다.
- 플레이헤드는 `+N분`으로 표시한 합성 시뮬레이션 시간이며 Live 위치 이동으로 오해할 모션을 사용하지 않는다. reduced-motion에서는 자동 진행을 멈춘다.
