...
2026-07-21 G5-B Round 1 사람 이해도 평가에서 2D·2.5D 모두 결정 목적, 경사 구간, 원·수신 기사 영향과 경로 순서를 충분히 전달하지 못했다. 따라서 관리자 활성 decision의 첫 정보위계를 `지금 답할 질문 → 현재·휴식·경사·지원 완료 순서 → 두 기사 영향 → 이유 → 동의·승인 상태`로 고정한다. 첫 질문은 `원 기사에게 10분 휴식과 배송 8건 이관을 진행할까요?`처럼 조치를 직접 묻고, `원 기사`는 작업이 줄어드는 기사, `수신 기사`는 배송을 받는 기사라는 뜻을 같은 카드에서 풀어 쓴다. 2.5D는 기본 화면이나 주요 행동이 아니라 `경사 근거 자세히 보기 · Demo 2.5D` 보조 상세로 낮춘다. 2026-07-23 Round 2에서도 경로 우선순위와 양측 영향 오인이 남아, 진행선 아래에 `10분 휴식이 예상 초과보다 먼저`라는 결론을 항상 표시하고 원 기사는 `8건 감소·안전여유 회복`, 수신 기사는 `8건 추가 후에도 기준 통과`처럼 결과부터 읽히게 고정한다. 2.5D도 같은 구조화 문구를 공유하며 모든 과거 round 증거를 별도 파일로 보존한다.
