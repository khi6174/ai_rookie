...
- 기사 카드, 지도 선택 스트립과 지원 큐에서는 개인 Safety Budget 원점수를 표시하지 않는다.
- 밴드와 `현재 한계 초과`, `한계까지 N분`, `향후 60분 내 초과 예상 없음`을 함께 사용한다.
...
- 지원 큐는 조치 마감 상태 순으로 보여주고 개인 순번·점수 랭킹을 제공하지 않는다.
- 이관 여력은 합성 집계, 필요 물량, 동일 위험 영향과 함께 표시한다. 대체 처방 계층은 추천 순위가 아니라 이관 불가 시 남는 안전한 선택지를 설명한다.
- 검증 상태는 별도 오버레이에서 미측정 성능, 잠정 운영 임계치, Fallback 입력, 알려진 한계와 다음 검증 계획의 미확정 상태를 보여준다.
...
- 공개 기본 경로 `/`는 단일 관제 데모를 표시하며 `/dashboard-demo`는 같은 화면의 호환 경로로 유지한다. 기존 관리자 폐루프 화면은 `/closed-loop-demo`에서 보존하고 헤더의 `폐루프 검증` 링크로 연결한다.
...
- 헤더는 왼쪽 `SR` 칩과 워드마크, 중앙 `Safety Control Tower`, 오른쪽 `합성 Demo / Live 0명 / 검증 상태 / 운영 관리자 / 15:28`을 같은 줄에 표시한다.
- 기사 현황 레일의 왼쪽에는 `조치 마감 임박 순 · 개인 성과 지표 아님`, 오른쪽에는 `전체·위험신호·지원·주의·정상` 필터와 이동 버튼을 같은 줄에 둔다. 필터는 카드를 제거하지 않고 비해당 카드의 투명도를 약 38%로 낮춘다.
- 카드에는 64px 합성 프로필 사진·이름·지정구역·밴드·조치 시각을 표시한다. 개인 Safety Budget 원점수는 표시하지 않는다. 1280×720 압축 레이아웃에서도 사진은 56px를 유지한다. 흰색, 1px 중립 경계, 16px 반경과 `--sh-flat`을 기본으로 하며 한쪽 강조선은 사용하지 않는다. 한계 초과 카드는 옅은 빨강 tint만 허용한다.
- 카드 상태는 아바타 우하단 상태점, 상태명 pill과 시간 문구를 함께 사용한다. 선택 카드는 상태색의 사방 2px 링을 사용한다. 빨강은 30 미만 한계 초과, 앰버는 지원 필요·주의, 그린은 정상에 사용한다.
- 지도에는 `위치 기준 15:28`, `Demo overlay`, 상태별 개수 범례, 권역별 합성 이관 여력과 상단 전역 `합성 Demo`, `Live 0명`을 제공한다. 마커는 움직이지 않고 겹치는 위치는 묶음 수와 대표 밴드를 함께 표시한다.
...
- 지도 좌하단에는 선택 기사의 상태, 이름, 구역, 조치 시각과 배송 진행을 요약한 칩을 고정한다.
...
- `지원 기준 45 미만` 목록은 9명이며 이름·구역·밴드·조치 시각을 표시하고 개인 순번과 Safety Budget 원점수를 표시하지 않는다. `조치 마감 상태 기준이며 기사 성과·평가가 아닙니다. 동의 전에는 현재 계획이 바뀌지 않습니다.` 안내를 파란 tint 배너로 고정한다.
- `검증 상태` 오버레이는 예측 성능 미측정, MVP 잠정 임계치, Fallback 입력, 알려진 한계와 파일럿 계획 미확정을 880px 이하 패널에서 보여준다.
...
- 개입·검증 모달은 흰색 평면, 사방이 같은 경계와 반투명 배경 오버레이를 사용한다. 후보 카드는 그림자 없이 전체 파란 경계와 옅은 파란 배경으로 선택을 표시하고, `Escape`, 배경 클릭, 닫기 버튼, 포커스 순환·복원을 지원한다.
