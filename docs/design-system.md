...
- 프로필 카드는 큰 정사각형 사진을 위에 두고 이름·지정구역·안전여유를 아래에 쌓는 세로형이다. 안전여유 숫자는 24px로 이름보다 크게 표시하고, 이모지·기호 없이 `초과·지원·주의·안정` 상태어를 함께 제공한다. 기사 성과 점수로 부르지 않는다. 소셜 프로필의 팔로워·인증·소개문·Follow 행동은 가져오지 않는다.
