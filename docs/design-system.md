...
`응급 상황 감지 예시`는 실제 센서 감지나 응급 신고 기능이 아니다. 버튼을 누르면 고정 합성 기사 `R-022`의 15분짜리 Demo 위험 신호만 같은 브라우저의 관제 화면에 전달한다. 성공 뒤에는 `대시보드에서 확인` 링크를 제공하며, 빨강과 함께 `매우 위험한 상태 감지`, `합성 예시 / 실제 신고 아님` 문구를 항상 표시한다.
