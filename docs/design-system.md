...
Shadow Live 준비 화면의 합성 배송 진행 재생은 별도 카드에서 `합성 실시간 재생 · 실제 TMS 아님` 배지를 제목 바로 위에 고정한다. 시작·일시정지·한 단계·초기화는 44px 이상 키보드 버튼으로 제공하고, 진행률은 숫자와 progress bar를 함께 사용한다. 기본 상태는 정지이며 자동재생으로 실제 운영처럼 보이게 하지 않는다. 공개 관리자 헤더의 진입 링크도 `합성 실시간 재생`이라고 명명하고 `Live` 단독 표현을 사용하지 않는다.
