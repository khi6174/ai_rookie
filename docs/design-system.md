...
- 지원 검토 모달은 `기사명 / 구역 | 배송 진행`, `지원 선택 / 5개`, 짧은 후보 결과와 `선택 사항`을 우선한다. 후보 카드는 조치명·조정 후 상태·배송 시간만 기본 표시하고 추천 또는 차단 이유만 한 줄로 덧붙인다. 이관 여력 부족 전환은 유지하며 대체 안내는 `분담 불가 시: 휴식 → 순서 변경 → 시간 재약정` 한 줄로 표시한다. 실제 지급·정산 미연동 표기는 바닥의 단일 합성 데모 문구로 합친다. 물량이관은 원 기사 확인 → 수신 기사 확인 → 관리자 승인 → 계획·ETA·고객안내 적용 순서를 유지한다.
