...
- 프로필 카드는 상단 왼쪽에 약 84~90px의 정사각형 사진, 오른쪽에 이름·지정구역을 밀착 배치하고, 하단에는 카드 폭 전체를 쓰는 46px 이상의 안전여유 숫자와 상태어를 `38.9 지원`처럼 한 줄로 묶는다. 별도의 하단 박스처럼 보이지 않도록 상태별 옅은 배경색을 카드 전체에 적용하고 내부 패딩은 6~7px, 상·하단 간격은 4px 수준으로 제한한다. 이모지·기호 없이 `긴급·지원·주의·안정` 상태어를 제공하고 기사 성과 점수로 부르지 않는다. 소셜 프로필의 팔로워·인증·소개문·Follow 행동은 가져오지 않는다.
