...
- 상단 중앙 제목은 `Safety Control Tower` 하나만 사용한다. 기사 레일 제목은 `기사 현황`이며 보조 문장 `지원 판단용 · 순위 아님`은 표시하지 않는다.
- 지도에는 `위치 기준 · 시각`과 상단 전역 `합성 Demo`, `Live 0`을 제공한다. 지도 안에서는 `합성 위치` 문구를 반복하지 않는다. 마커는 움직이지 않고 겹치는 위치는 묶음 수와 그 안의 최저 Safety Budget을 함께 표시한다.
...
- 기사 현황 레일에는 `전체·위험신호·지원·주의·안정` 필터를 제공한다. 필터 수치는 지원 판단을 위한 상태 집계이며 기사 순위가 아니다.
- 기사 앱에서 명시적인 현재 위험 신호가 수신된 경우에만 해당 기사 카드 전체를 빨간색으로 전환한다. 낮은 Safety Budget만으로 카드 전체를 빨갛게 만들지 않는다. 빨간 카드에는 흰색 `기사앱 위험 신호` 문구와 안전여유 숫자를 함께 표시하며, 그림자나 한쪽 강조선을 사용하지 않는다.
- 우측 `지원 검토` 패널은 `선택 기사`, 지정구역, 안전여유, `안전한계`, 배송 완료 건수와 `지원안 검토`를 제공한다. `지원안 검토`는 다른 경로로 이동하지 않고 같은 화면의 모달을 열어 후보 비교 → 기사 확인·수정·거절 → 관리자 승인 → 계획·ETA·고객안내 적용을 끝낸다. `지원 기준 45 미만`의 `지원 필요` 목록은 같은 기사·지도 선택 상태를 사용하며 승인 전 현재 계획 유지 문구를 고정한다.
- 색감과 패널 문법은 공개 루트 관리자 대시보드와 동일하게 `#F4F7FC` 페이지, 흰 패널, `#E6ECF5` 경계, `#2563EB` 주요 행동·선택, `#0B8F43` 안전·운영 강조를 사용한다. 헤더는 흰색으로 유지하고 다크 네이비 전면 헤더와 청록 브랜드 포인트를 사용하지 않는다.
- 루트 대시보드의 시각 언어를 적용하더라도 기사 카드·안전여유 면·지원 큐에는 그림자나 한쪽 강조선을 추가하지 않는다. 선택은 `#2563EB` 사방 경계와 `#EAF0FF` 배경으로 표시한다.
- 프로필 카드는 상단 왼쪽에 약 84~90px의 정사각형 사진, 오른쪽에 이름·지정구역을 밀착 배치하고, 하단에는 카드 폭 전체를 쓰는 46px 이상의 안전여유 숫자를 배치한다. 별도의 하단 박스처럼 보이지 않도록 상태별 옅은 배경색을 카드 전체에 적용하고 내부 패딩은 6~7px, 상·하단 간격은 4px 수준으로 제한한다. `안전여유` 라벨은 상단, 숫자는 카드 하단에 고정하며 숫자 뒤의 `긴급·지원·주의·안정` 상태어는 카드에서 표시하지 않는다. 상태는 숫자·카드색·필터·접근성 이름으로 유지하고 기사 성과 점수로 부르지 않는다. 소셜 프로필의 팔로워·인증·소개문·Follow 행동은 가져오지 않는다.
- 카드와 안전여유 면에는 그림자·필터·상태색 세로선을 사용하지 않는다. 기본 카드는 얇은 중립 테두리로 구분하고 선택 상태는 사방의 동일한 파란 테두리로 표시한다.
- 실제 지도와 Fallback 지도의 개별 기사 마커는 이름 글자 대신 해당 합성 프로필 사진을 사용한다. 마커는 지도 위에 떠 있는 42px 원형 사진, 흰색 전체 테두리, 우상단 상태점과 우하단 안전여유 배지로 구성한다. 선택은 사진 전체의 파란 외곽선으로 표시하며 마커에도 그림자나 한쪽 강조선을 사용하지 않는다.
...
- 기본 높이는 헤더 54px, 카드 영역 296px, 지도 가변이다. 1280×720에서는 헤더 50px, 카드 영역 266px와 카드 높이 216px를 사용한다.
