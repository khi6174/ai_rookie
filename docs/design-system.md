...
- 기사 마커는 현재 초를 입력으로 한 가상 위치를 1초마다 갱신한다. 각 마커는 강남권 육지의 미리 지정한 짧은 도로 구간 안에서만 왕복한다. 지도 툴바에는 `위치 갱신`으로 표시하고 화면 상단의 `시연 데이터` 한 곳에서 출처 경계를 전달한다.
