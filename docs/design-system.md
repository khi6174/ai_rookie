...
- 1-1의 `강수·시정을 1-2에 적용`은 반영된 강수·시정과 예상 최저·안전한계 시점·추천의 전후 결과를 1-2 상단에 보여준다. 강수·시정 slider에는 `관측 적용됨`을 표시하고 사용자가 값을 다시 조정하면 제거한다. 관측 기온은 체감온도로 임의 변환하지 않았음을 함께 명시한다.
