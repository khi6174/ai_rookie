...
관리자 지도는 `design-v2.0.0`의 G2-A 범위를 반영했지만 전체 완성 화면은 아니다. 지도 오류 목록 fallback·접근성 E2E와 실제 지도 공급자는 미구현이며, 기사 화면은 여전히 반응형 모바일 웹과 compact Fallback map이다. 이 합성 fixture 경계는 후속 지도·PWA 단계가 실제 검증되기 전까지 정직하게 유지한다.
