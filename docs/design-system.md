...
- 공개 Demo에 승인된 Kakao Maps JavaScript 키가 있으면 실제 강남 도로 베이스맵 위에 결정론적 합성 기사 좌표와 강남 허브를 표시한다. `Kakao 지도 · 합성 위치`를 명시하며, 키·도메인·네트워크 오류에서는 같은 선택 상태를 유지한 채 기존 Fallback 지도로 자동 복귀한다.
...
- 우측 지원 판단 패널은 선택 기사, 지정구역, 안전여유, 임계치 예상, 배송 진행과 `개입 검토 열기`를 제공한다. `개입 검토 열기`는 다른 경로로 이동하지 않고 같은 화면의 모달을 열어 후보 비교 → 기사 확인·수정·거절 → 관리자 승인 → 계획·ETA·고객안내 적용을 끝낸다. 임계치 45 미만 지원 큐는 같은 기사·지도 선택 상태를 사용하며 승인 전 현재 계획 유지 문구를 고정한다.
...
- 허브 위치는 의미가 불명확한 단일 `H` 문자를 사용하지 않는다. 네이비 건물 형태의 CSS 아이콘과 `강남 허브` 텍스트를 흰색 가로형 표식 안에 함께 표시하고, 접근성 이름에는 합성 위치임을 포함한다.
