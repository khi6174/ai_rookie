...
test("G5-B Round 4는 시간·지점·조치·양측 영향을 풀어 쓴 같은 decision의 2D와 보조 2.5D 자극을 고정한다", async ({ browser, page }) => {
  ...
  // ADR-142's verified explanation disclosure, ADR-143's A.X v2 evidence
  ...
  // disabled server-Gate disclosure and ADR-151's synthetic delivery stream
  // controls are part of the non-spatial baseline.
  ...
  const hasKakaoJavascriptBuild = Boolean(
    process.env.VITE_KAKAO_MAP_JAVASCRIPT_KEY?.trim()
      || /^VITE_KAKAO_MAP_JAVASCRIPT_KEY\s*=\s*\S+/m.test(localEnv),
  );
  const previousG4BGzipJsBytes = 155_422 + (hasKakaoJavascriptBuild ? 2_867 : 0);
