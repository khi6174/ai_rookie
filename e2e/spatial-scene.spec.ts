...
test("G5-B Round 4는 시간·지점·조치·양측 영향을 풀어 쓴 같은 decision의 2D와 보조 2.5D 자극을 고정한다", async ({ browser, page }) => {
  ...
  // Q&A, ADR-145's courier intake UI and ADR-146's decision-first dashboard
  // hierarchy are part of the non-spatial baseline.
  ...
  const previousG4BGzipJsBytes = 149_956 + (hasKakaoJavascriptBuild ? 2_867 : 0);
